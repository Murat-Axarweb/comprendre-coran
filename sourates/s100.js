// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S100 = {
  "meta": {
    "numero": 100,
    "nom_ar": "العاديات",
    "nom_translit": "Al-'Adiyat",
    "nom_fr": "Les Coursiers",
    "nom_en": "The Chargers",
    "nom_tr": "Âdiyât (Koşan Atlar)",
    "type": "mecquoise",
    "versets_count": 11,
    "theme": "Du galop des chevaux de guerre à l'ingratitude du cœur humain : ce que la tombe révélera",
    "note_pedagogique": "Cinq serments haletants — une charge de cavalerie à l'aube — pour introduire un diagnostic : l'homme est kanūd, ingrat envers son Seigneur, et il en est lui-même témoin. Le rythme de la sourate mime le galop qu'elle décrit.",
    "progression": {
      "couche": 2,
      "prerequis": [
        99
      ],
      "suivant": [
        101
      ]
    },
    "racines_cles": [
      "ع-د-و",
      "ك-ن-د",
      "ح-ب-ب",
      "ق-ب-ر",
      "ص-د-ر"
    ],
    "traduction_note": "Traductions pédagogiques originales (FR/EN/TR) — Comprendre le Coran"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "وَالْعَٰدِيَٰتِ ضَبْحًا",
      "translit": "Wal-ʿādiyāti ḍabḥā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Par les coursiers qui halètent"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "By the racers, panting"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "And olsun Allah yolunda koştukça koşanlara"
        }
      },
      "mots": [],
      "niveau_couche": 2
    },
    {
      "numero": 2,
      "ar": "فَالْمُورِيَٰتِ قَدْحًا",
      "translit": "Fal-mūriyāti qadḥā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "qui font jaillir des étincelles"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And the producers of sparks [when] striking"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "And olsun kıvılcımlar saçanlara"
        }
      },
      "mots": [],
      "niveau_couche": 2
    },
    {
      "numero": 3,
      "ar": "فَالْمُغِيرَٰتِ صُبْحًا",
      "translit": "Fal-mughīrāti ṣubḥā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "qui attaquent au matin"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And the chargers at dawn"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Sabah sabah akına çıkanlara"
        }
      },
      "mots": [],
      "niveau_couche": 2
    },
    {
      "numero": 4,
      "ar": "فَأَثَرْنَ بِهِ نَقْعًا",
      "translit": "Faʾatharna bihī naqʿā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et font ainsi voler la poussière"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Stirring up thereby [clouds of] dust"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Ve tozu dumana katanlara"
        }
      },
      "mots": [
        {
          "ar": "بِهِ",
          "translit": "bihī",
          "fr": "par lui",
          "racine": "بِ + هُ",
          "frequence_coran": 327
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 5,
      "ar": "فَوَسَطْنَ بِهِ جَمْعًا",
      "translit": "Fawasaṭna bihī jamʿā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et pénètrent au centre de la troupe ennemie"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Arriving thereby in the center collectively"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Düşman topluluğunun içine dalanlara ki"
        }
      },
      "mots": [
        {
          "ar": "بِهِ",
          "translit": "bihī",
          "fr": "par lui",
          "racine": "بِ + هُ",
          "frequence_coran": 327
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 6,
      "ar": "إِنَّ الْإِنسَٰنَ لِرَبِّهِ لَكَنُودٌ",
      "translit": "Inna l-ʾinsāna lirabbihī lakanūd",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "L'homme est, certes, ingrat envers son Seigneur"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Indeed mankind, to his Lord, is ungrateful"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İnsan gerçekten Rabbine karşı pek nankördür"
        }
      },
      "mots": [
        {
          "ar": "إِنَّ",
          "translit": "inna",
          "fr": "certes",
          "racine": "particule d’insistance",
          "frequence_coran": 609
        },
        {
          "ar": "لِرَبِّهِ",
          "translit": "lirabbihī",
          "fr": "son Seigneur",
          "racine": "ر-ب-ب",
          "frequence_coran": 44
        }
      ],
      "analyse_globale": "Kanūd : l'ingrat qui compte les malheurs et oublie les dons — le mot désignait aussi la terre stérile qui ne rend rien de ce qu'on y sème. Le contraste avec les serments est cinglant : le cheval, lui, s'épuise loyalement pour son maître ; l'homme, comblé, renie le sien.",
      "niveau_couche": 2
    },
    {
      "numero": 7,
      "ar": "وَإِنَّهُ عَلَىٰ ذَٰلِكَ لَشَهِيدٌ",
      "translit": "Waʾinnahū ʿalā dhālika lashahīd",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et pourtant, il est certes, témoin de cela"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And indeed, he is to that a witness"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Doğrusu kendisi de bunların hepsine şahittir"
        }
      },
      "mots": [
        {
          "ar": "وَإِنَّهُ",
          "translit": "waʾinnahū",
          "fr": "certes il",
          "racine": "إِنَّ + هُ",
          "frequence_coran": 147
        },
        {
          "ar": "عَلَىٰ",
          "translit": "ʿalā",
          "fr": "sur",
          "racine": "particule",
          "frequence_coran": 1430
        },
        {
          "ar": "ذَٰلِكَ",
          "translit": "dhālika",
          "fr": "cela",
          "racine": "pronom démonstratif",
          "frequence_coran": 280
        },
        {
          "ar": "لَشَهِيدٌ",
          "translit": "lashahīdun",
          "fr": "témoin",
          "racine": "ش-ه-د",
          "frequence_coran": 35
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 8,
      "ar": "وَإِنَّهُ لِحُبِّ الْخَيْرِ لَشَدِيدٌ",
      "translit": "Waʾinnahū liḥubbi l-khayri lashadīd",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et pour l'amour des richesses il est certes ardent"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And indeed he is, in love of wealth, intense"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Gerçekten mala da pek düşkündür"
        }
      },
      "mots": [
        {
          "ar": "وَإِنَّهُ",
          "translit": "waʾinnahū",
          "fr": "certes il",
          "racine": "إِنَّ + هُ",
          "frequence_coran": 147
        },
        {
          "ar": "لِحُبِّ",
          "translit": "liḥubbi",
          "fr": "grain",
          "racine": "ح-ب-ب",
          "frequence_coran": 7
        },
        {
          "ar": "الْخَيْرِ",
          "translit": "l-khayri",
          "fr": "meilleur",
          "racine": "خ-ي-ر",
          "frequence_coran": 78
        },
        {
          "ar": "لَشَدِيدٌ",
          "translit": "lashadīdun",
          "fr": "intense",
          "racine": "ش-د-د",
          "frequence_coran": 52
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 9,
      "ar": "أَفَلَا يَعْلَمُ إِذَا بُعْثِرَ مَا فِي الْقُبُورِ",
      "translit": "Afalā yaʿlamu idhā buʿthira mā fī l-qubūr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Ne sait-il donc pas que lorsque ce qui est dans les tombes sera bouleversé"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "But does he not know that when the contents of the graves are scattered"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İnsan, kabirlerde bulunanların çıkarılacağı ve kalblerde olanların ortaya konulacağı bir zamanın geleceğini bilmez mi"
        }
      },
      "mots": [
        {
          "ar": "أَفَلَا",
          "translit": "afalā",
          "fr": "ne... donc pas ?",
          "racine": "أ + ف + لَا",
          "frequence_coran": 45
        },
        {
          "ar": "يَعْلَمُ",
          "translit": "yaʿlamu",
          "fr": "il sait",
          "racine": "ع-ل-م",
          "frequence_coran": 62
        },
        {
          "ar": "إِذَا",
          "translit": "idhā",
          "fr": "quand",
          "racine": "particule temporelle",
          "frequence_coran": 195
        },
        {
          "ar": "مَا",
          "translit": "mā",
          "fr": "ce que",
          "racine": "particule",
          "frequence_coran": 1010
        },
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "dans",
          "racine": "particule",
          "frequence_coran": 4381
        },
        {
          "ar": "الْقُبُورِ",
          "translit": "l-qubūri",
          "fr": "tombes",
          "racine": "ق-ب-ر",
          "frequence_coran": 5
        }
      ],
      "analyse_globale": "Buʿthira mā fī l-qubūr / ḥuṣṣila mā fī ṣ-ṣudūr : deux dévoilements parallèles — les tombes rendront les corps, les poitrines rendront les secrets. La rime arabe (qubūr/ṣudūr) lie les deux exhumations : celle des morts et celle des intentions.",
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 9,
          "fin": 10,
          "principal": true
        }
      }
    },
    {
      "numero": 10,
      "ar": "وَحُصِّلَ مَا فِي الصُّدُورِ",
      "translit": "Waḥuṣṣila mā fī ṣ-ṣudūr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et que sera dévoilé ce qui est dans les poitrines"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And that within the breasts is obtained"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İnsan, kabirlerde bulunanların çıkarılacağı ve kalblerde olanların ortaya konulacağı bir zamanın geleceğini bilmez mi"
        }
      },
      "mots": [
        {
          "ar": "مَا",
          "translit": "mā",
          "fr": "ce que",
          "racine": "particule",
          "frequence_coran": 1010
        },
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "dans",
          "racine": "particule",
          "frequence_coran": 4381
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 9,
          "fin": 10,
          "principal": false
        }
      }
    },
    {
      "numero": 11,
      "ar": "إِنَّ رَبَّهُم بِهِمْ يَوْمَئِذٍ لَّخَبِيرٌ",
      "translit": "Inna rabbahum bihim yawmaʾidhin lakhabīr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "ce jour-là, certes, leur Seigneur sera Parfaitement Connaisseur d'eux"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Indeed, their Lord with them, that Day, is [fully] Acquainted"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Doğrusu Rableri o gün onların her şeyinden haberdardır"
        }
      },
      "mots": [
        {
          "ar": "إِنَّ",
          "translit": "inna",
          "fr": "certes",
          "racine": "particule d’insistance",
          "frequence_coran": 609
        },
        {
          "ar": "رَبَّهُم",
          "translit": "rabbahum",
          "fr": "leur Seigneur",
          "racine": "ر-ب-ب",
          "frequence_coran": 80
        },
        {
          "ar": "بِهِمْ",
          "translit": "bihim",
          "fr": "eux",
          "racine": "pronom personnel",
          "frequence_coran": 180
        },
        {
          "ar": "يَوْمَئِذٍ",
          "translit": "yawmaʾidhin",
          "fr": "ce jour-là",
          "racine": "ي-و-م + إِذْ",
          "frequence_coran": 63
        },
        {
          "ar": "لَّخَبِيرٌ",
          "translit": "lakhabīru",
          "fr": "Parfaitement Informé",
          "racine": "خ-ب-ر",
          "frequence_coran": 38
        }
      ],
      "niveau_couche": 2
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "الْعَٰدِيَٰت",
      "لَكَنُود",
      "حُبّ الْخَيْر",
      "بُعْثِرَ",
      "الصُّدُور"
    ],
    "racines_a_retenir": [
      "ش-ه-د",
      "ح-ب-ب",
      "ب-ع-ث-ر"
    ],
    "declic_frequence": "ḥubb al-khayr (v.8) : khayr signifie 'le bien' mais désigne ici les biens matériels — le même mot pour la vertu et la richesse. L'ambiguïté est voulue : l'homme aime 'le bien'... au sens qui l'arrange. Son amour des biens est shadīd, intense."
  }
};

export { S100 };
