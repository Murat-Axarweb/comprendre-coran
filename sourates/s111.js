// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S111 = {
  "meta": {
    "numero": 111,
    "nom_ar": "المسد",
    "nom_translit": "Al-Masad",
    "nom_fr": "Les Fibres",
    "nom_en": "The Palm Fibre",
    "nom_tr": "Mesed (Alev)",
    "type": "mecquoise",
    "versets_count": 5,
    "theme": "La condamnation nominale d'Abu Lahab : ni fortune ni rang ne protègent de la justice divine",
    "note_pedagogique": "Seul adversaire nommé dans tout le Coran : Abu Lahab, oncle du Prophète. La sourate prophétise qu'il mourra mécréant — il aurait pu la démentir en se convertissant, il ne l'a jamais fait. Un défi resté ouvert dix ans.",
    "progression": {
      "couche": 1,
      "prerequis": [
        110
      ],
      "suivant": [
        97
      ]
    },
    "racines_cles": [
      "ت-ب-ب",
      "غ-ن-ي",
      "ك-س-ب",
      "ل-ه-ب",
      "ح-م-ل"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "تَبَّتْ يَدَا أَبِي لَهَبٍ وَتَبَّ",
      "translit": "Tabbat yadā abī lahabin watabb",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Que périssent les deux mains d'Abû-Lahab et que lui-même périsse"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "May the hands of Abu Lahab be ruined, and ruined is he"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Ebu Leheb'in elleri kurusun; kurudu da"
        }
      },
      "mots": [
        {
          "ar": "تَبَّتْ",
          "translit": "tabbat",
          "fr": "que périssent",
          "racine": "ت-ب-ب",
          "frequence_coran": 3
        },
        {
          "ar": "يَدَا",
          "translit": "yadā",
          "fr": "les deux mains (de)",
          "racine": "ي-د-ي",
          "frequence_coran": 120
        },
        {
          "ar": "أَبِي",
          "translit": "abī",
          "fr": "père (de)",
          "racine": "أ-ب-و",
          "frequence_coran": 117
        },
        {
          "ar": "لَهَبٍ",
          "translit": "lahabin",
          "fr": "Lahab (la Flamme)",
          "racine": "ل-ه-ب",
          "frequence_coran": 3
        },
        {
          "ar": "وَتَبَّ",
          "translit": "watabba",
          "fr": "et il a péri",
          "racine": "ت-ب-ب",
          "frequence_coran": 3
        }
      ],
      "analyse_globale": "Abu Lahab signifie 'père de la Flamme' — surnom donné pour son teint éclatant. Le verset 3 retourne le surnom contre lui : il entrera dans un feu 'dhāta lahab', plein de flammes. Son nom même devient sa sentence : ironie prophétique intraduisible.",
      "niveau_couche": 1
    },
    {
      "numero": 2,
      "ar": "مَا أَغْنَىٰ عَنْهُ مَالُهُ وَمَا كَسَبَ",
      "translit": "Mā aghnā ʿanhu māluhū wamā kasab",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Sa fortune ne lui sert à rien, ni ce qu'il a acquis"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "His wealth will not avail him or that which he gained"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Malı ve kazandığı kendisine fayda vermedi"
        }
      },
      "mots": [
        {
          "ar": "مَا",
          "translit": "mā",
          "fr": "ne... pas",
          "racine": "— (négation)",
          "frequence_coran": 0
        },
        {
          "ar": "أَغْنَىٰ",
          "translit": "aghnā",
          "fr": "lui a servi",
          "racine": "غ-ن-ي",
          "frequence_coran": 73
        },
        {
          "ar": "عَنْهُ",
          "translit": "ʿanhu",
          "fr": "à lui",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "مَالُهُ",
          "translit": "māluhū",
          "fr": "sa fortune",
          "racine": "م-و-ل",
          "frequence_coran": 86
        },
        {
          "ar": "وَمَا",
          "translit": "wamā",
          "fr": "ni ce que",
          "racine": "— (relatif)",
          "frequence_coran": 0
        },
        {
          "ar": "كَسَبَ",
          "translit": "kasaba",
          "fr": "il a acquis",
          "racine": "ك-س-ب",
          "frequence_coran": 67
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 3,
      "ar": "سَيَصْلَىٰ نَارًا ذَاتَ لَهَبٍ",
      "translit": "Sayaṣlā nāran dhāta lahab",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Il sera brûlé dans un Feu plein de flammes"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "He will [enter to] burn in a Fire of [blazing] flame"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Alevli ateşe yaslanacaktır"
        }
      },
      "mots": [
        {
          "ar": "سَيَصْلَىٰ",
          "translit": "sayaṣlā",
          "fr": "il brûlera bientôt",
          "racine": "ص-ل-ي",
          "frequence_coran": 25
        },
        {
          "ar": "نَارًا",
          "translit": "nāran",
          "fr": "dans un feu",
          "racine": "ن-و-ر",
          "frequence_coran": 145
        },
        {
          "ar": "ذَاتَ",
          "translit": "dhāta",
          "fr": "doté de / plein de",
          "racine": "— (nom)",
          "frequence_coran": 0
        },
        {
          "ar": "لَهَبٍ",
          "translit": "lahabin",
          "fr": "flammes",
          "racine": "ل-ه-ب",
          "frequence_coran": 3
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 4,
      "ar": "وَامْرَأَتُهُ حَمَّالَةَ الْحَطَبِ",
      "translit": "Wamraʾatuhū ḥammālata l-ḥaṭab",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "de même sa femme, la porteuse de bois"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And his wife [as well] - the carrier of firewood"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Karısı da, boynunda bir ip olduğu halde ona odun taşıyacaktır"
        }
      },
      "mots": [
        {
          "ar": "وَامْرَأَتُهُ",
          "translit": "wamraʾatuhū",
          "fr": "et sa femme",
          "racine": "م-ر-أ",
          "frequence_coran": 38
        },
        {
          "ar": "حَمَّالَةَ",
          "translit": "ḥammālata",
          "fr": "porteuse (de)",
          "racine": "ح-م-ل",
          "frequence_coran": 64
        },
        {
          "ar": "الْحَطَبِ",
          "translit": "l-ḥaṭabi",
          "fr": "bois",
          "racine": "ح-ط-ب",
          "frequence_coran": 2
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 4,
          "fin": 5,
          "principal": true
        }
      }
    },
    {
      "numero": 5,
      "ar": "فِي جِيدِهَا حَبْلٌ مِّن مَّسَدٍ",
      "translit": "Fī jīdihā ḥablun min masad",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "à son cou, une corde de fibres"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Around her neck is a rope of [twisted] fiber"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Karısı da, boynunda bir ip olduğu halde ona odun taşıyacaktır"
        }
      },
      "mots": [
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "à / dans",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "جِيدِهَا",
          "translit": "jīdihā",
          "fr": "son cou",
          "racine": "ج-ي-د",
          "frequence_coran": 1
        },
        {
          "ar": "حَبْلٌ",
          "translit": "ḥablun",
          "fr": "une corde",
          "racine": "ح-ب-ل",
          "frequence_coran": 7
        },
        {
          "ar": "مِّن",
          "translit": "min",
          "fr": "de",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "مَّسَدٍ",
          "translit": "masadi",
          "fr": "fibres tressées",
          "racine": "م-س-د",
          "frequence_coran": 1
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 4,
          "fin": 5,
          "principal": false
        }
      }
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "تَبَّتْ",
      "لَهَب",
      "كَسَبَ",
      "حَبْل"
    ],
    "racines_a_retenir": [
      "ك-س-ب",
      "م-و-ل",
      "ل-ه-ب"
    ],
    "declic_frequence": "kasaba (acquérir, racine ك-س-ب ~67 occurrences) est le grand mot du bilan personnel : 'à chaque âme ce qu'elle a acquis'. Ici, ni la fortune (māl) ni les acquis d'Abu Lahab ne lui servent — première leçon d'économie coranique : tout kasb sans foi est un actif gelé."
  }
};

export { S111 };
