const S108 = {
  "meta": {
    "numero": 108,
    "nom_ar": "الكوثر",
    "nom_translit": "Al-Kawthar",
    "nom_fr": "L'Abondance",
    "nom_en": "Abundance",
    "nom_tr": "Kevser",
    "type": "mecquoise",
    "versets_count": 3,
    "theme": "La sourate la plus courte : consolation du Prophète par le don de l'abondance",
    "note_pedagogique": "Trois versets révélés quand les ennemis du Prophète le traitaient d'abtar (sans descendance, sans avenir) après la mort de ses fils. Réponse divine : c'est l'insulteur qui sera sans postérité.",
    "progression": {
      "couche": 1,
      "prerequis": [
        103
      ],
      "suivant": [
        110
      ]
    },
    "racines_cles": [
      "ك-ث-ر",
      "ص-ل-و",
      "ن-ح-ر",
      "ب-ت-ر"
    ],
    "traduction_note": "Traductions pédagogiques originales (FR/EN/TR) — Comprendre le Coran"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "إِنَّا أَعْطَيْنَٰكَ الْكَوْثَرَ",
      "translit": "Innā aʿṭaynāka l-kawthar",
      "traductions": {
        "fr": {
          "auteur": "Comprendre le Coran",
          "texte": "Nous t'avons certes donné l'Abondance."
        },
        "en": {
          "auteur": "Comprendre le Coran",
          "texte": "We have surely given you Abundance."
        },
        "tr": {
          "auteur": "Comprendre le Coran",
          "texte": "Şüphesiz Biz sana Kevser'i verdik."
        }
      },
      "mots": [
        {
          "ar": "إِنَّا",
          "translit": "innā",
          "fr": "certes Nous",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "أَعْطَيْنَٰكَ",
          "translit": "aʿṭaynāka",
          "fr": "Nous t'avons donné",
          "racine": "ع-ط-و",
          "frequence_coran": 14
        },
        {
          "ar": "الْكَوْثَرَ",
          "translit": "l-kawthara",
          "fr": "l'Abondance",
          "racine": "ك-ث-ر",
          "frequence_coran": 167
        }
      ],
      "analyse_globale": "Al-Kawthar désigne à la fois le fleuve du Paradis promis au Prophète et, plus largement, l'abondance de tout bien : la révélation, la descendance spirituelle, le renom. L'histoire a tranché : le nom de celui qu'on disait 'sans avenir' est prononcé chaque seconde quelque part sur terre.",
      "niveau_couche": 1
    },
    {
      "numero": 2,
      "ar": "فَصَلِّ لِرَبِّكَ وَانْحَرْ",
      "translit": "Faṣalli lirabbika wanḥar",
      "traductions": {
        "fr": {
          "auteur": "Comprendre le Coran",
          "texte": "Prie donc ton Seigneur et sacrifie."
        },
        "en": {
          "auteur": "Comprendre le Coran",
          "texte": "So pray to your Lord and sacrifice."
        },
        "tr": {
          "auteur": "Comprendre le Coran",
          "texte": "Öyleyse Rabbin için namaz kıl ve kurban kes."
        }
      },
      "mots": [
        {
          "ar": "فَصَلِّ",
          "translit": "faṣalli",
          "fr": "prie donc",
          "racine": "ص-ل-و",
          "frequence_coran": 99
        },
        {
          "ar": "لِرَبِّكَ",
          "translit": "lirabbika",
          "fr": "pour ton Seigneur",
          "racine": "ر-ب-ب",
          "frequence_coran": 970
        },
        {
          "ar": "وَانْحَرْ",
          "translit": "wanḥar",
          "fr": "et sacrifie",
          "racine": "ن-ح-ر",
          "frequence_coran": 1
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 3,
      "ar": "إِنَّ شَانِئَكَ هُوَ الْأَبْتَرُ",
      "translit": "Inna shāniʾaka huwa l-ʾabtar",
      "traductions": {
        "fr": {
          "auteur": "Comprendre le Coran",
          "texte": "Celui qui te hait — c'est lui l'amputé de tout avenir."
        },
        "en": {
          "auteur": "Comprendre le Coran",
          "texte": "Indeed your enemy is the one cut off."
        },
        "tr": {
          "auteur": "Comprendre le Coran",
          "texte": "Asıl sana kin besleyendir soyu kesik olan."
        }
      },
      "mots": [
        {
          "ar": "إِنَّ",
          "translit": "inna",
          "fr": "certes",
          "racine": "— (particule)",
          "frequence_coran": 1536
        },
        {
          "ar": "شَانِئَكَ",
          "translit": "shāniʾaka",
          "fr": "celui qui te hait",
          "racine": "ش-ن-أ",
          "frequence_coran": 2
        },
        {
          "ar": "هُوَ",
          "translit": "huwa",
          "fr": "c'est lui",
          "racine": "— (pronom)",
          "frequence_coran": 657
        },
        {
          "ar": "الْأَبْتَرُ",
          "translit": "l-ʾabtaru",
          "fr": "l'amputé / sans postérité",
          "racine": "ب-ت-ر",
          "frequence_coran": 1
        }
      ],
      "niveau_couche": 1
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "الْكَوْثَر",
      "صَلِّ",
      "انْحَرْ",
      "الْأَبْتَر"
    ],
    "racines_a_retenir": [
      "ك-ث-ر",
      "ص-ل-و"
    ],
    "declic_frequence": "kawthar est une forme intensive de kathīr (beaucoup, racine ك-ث-ر ~167 occurrences) : non pas 'beaucoup' mais 'l'abondance surabondante'. Face à la perte et à la moquerie, la réponse divine est un mot construit sur l'excès du don."
  }
};

export { S108 };
