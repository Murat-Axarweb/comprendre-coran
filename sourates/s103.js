// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S103 = {
  "meta": {
    "numero": 103,
    "nom_ar": "العصر",
    "nom_translit": "Al-'Asr",
    "nom_fr": "Le Temps",
    "nom_en": "Time",
    "nom_tr": "Asr (Zaman)",
    "type": "mecquoise",
    "versets_count": 3,
    "theme": "La formule du salut en trois versets : foi, œuvres, vérité, patience",
    "note_pedagogique": "L'imam ash-Shafi'i disait : si les gens méditaient seulement cette sourate, elle leur suffirait. Trois versets qui posent le diagnostic (perte) et le remède (quatre conditions).",
    "progression": {
      "couche": 1,
      "prerequis": [
        112
      ],
      "suivant": [
        104,
        108
      ]
    },
    "racines_cles": [
      "ع-ص-ر",
      "خ-س-ر",
      "ص-ب-ر",
      "ح-ق-ق"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "وَالْعَصْرِ",
      "translit": "Wal-ʿaṣr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Par le Temps"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "By time"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İkindi vaktine (Asra; çağa) and olsun ki"
        }
      },
      "mots": [
        {
          "ar": "وَالْعَصْرِ",
          "translit": "wal-ʿaṣri",
          "fr": "par le Temps !",
          "racine": "ع-ص-ر",
          "frequence_coran": 2
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 2,
      "ar": "إِنَّ الْإِنسَٰنَ لَفِي خُسْرٍ",
      "translit": "Inna l-ʾinsāna lafī khusr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "L'homme est certes, en perdition"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Indeed, mankind is in loss"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İnsan hiç şüphesiz hüsran içindedir"
        }
      },
      "mots": [
        {
          "ar": "إِنَّ",
          "translit": "inna",
          "fr": "certes",
          "racine": "— (particule)",
          "frequence_coran": 1536
        },
        {
          "ar": "الْإِنسَٰنَ",
          "translit": "l-ʾinsāna",
          "fr": "l'homme",
          "racine": "أ-ن-س",
          "frequence_coran": 65
        },
        {
          "ar": "لَفِي",
          "translit": "lafī",
          "fr": "est bien dans",
          "racine": "— (emphase + fī)",
          "frequence_coran": 0
        },
        {
          "ar": "خُسْرٍ",
          "translit": "khusrin",
          "fr": "une perdition",
          "racine": "خ-س-ر",
          "frequence_coran": 65
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 3,
      "ar": "إِلَّا الَّذِينَ ءَامَنُواْ وَعَمِلُواْ الصَّـٰلِحَٰتِ وَتَوَاصَوْاْ بِالْحَقِّ وَتَوَاصَوْاْ بِالصَّبْرِ",
      "translit": "Illā lladhīna āmanū waʿamilū ṣ-ṣāliḥāti watawāṣaw bil-ḥaqqi watawāṣaw biṣ-ṣabr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "sauf ceux qui croient et accomplissent les bonnes œuvres, s'enjoignent mutuellement la vérité et s'enjoignent mutuellement l'endurance"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Except for those who have believed and done righteous deeds and advised each other to truth and advised each other to patience"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Ancak inanıp yararlı iş işleyenler, birbirlerine gerçeği tavsiye edenler ve sabırlı olmayı tavsiye edenler bunun dışındadır"
        }
      },
      "mots": [
        {
          "ar": "إِلَّا",
          "translit": "illā",
          "fr": "sauf",
          "racine": "— (exception)",
          "frequence_coran": 663
        },
        {
          "ar": "الَّذِينَ",
          "translit": "lladhīna",
          "fr": "ceux qui",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "ءَامَنُواْ",
          "translit": "āmanū",
          "fr": "ont cru",
          "racine": "أ-م-ن",
          "frequence_coran": 879
        },
        {
          "ar": "وَعَمِلُواْ",
          "translit": "waʿamilū",
          "fr": "et ont accompli",
          "racine": "ع-م-ل",
          "frequence_coran": 360
        },
        {
          "ar": "الصَّـٰلِحَٰتِ",
          "translit": "ṣ-ṣāliḥāti",
          "fr": "les œuvres bonnes",
          "racine": "ص-ل-ح",
          "frequence_coran": 180
        },
        {
          "ar": "وَتَوَاصَوْاْ",
          "translit": "watawāṣaw",
          "fr": "et se sont recommandé",
          "racine": "و-ص-ي",
          "frequence_coran": 32,
          "famille": [
            "وَصِيَّة (waṣiyya) — testament",
            "وَصَّىٰ (waṣṣā) — recommander",
            "تَوَاصٍ (tawāṣin) — recommandation mutuelle"
          ],
          "profondeur": "Tawāṣaw est une forme réciproque (forme VI) : ce n'est pas un maître qui recommande à des élèves, mais des égaux qui se soutiennent mutuellement. Le salut décrit ici n'est pas individuel : croire et bien agir ne suffisent pas sans une communauté où circule le rappel de la vérité et de la patience."
        },
        {
          "ar": "بِالْحَقِّ",
          "translit": "bil-ḥaqqi",
          "fr": "la vérité",
          "racine": "ح-ق-ق",
          "frequence_coran": 287
        },
        {
          "ar": "وَتَوَاصَوْاْ",
          "translit": "watawāṣaw",
          "fr": "et se sont recommandé",
          "racine": "و-ص-ي",
          "frequence_coran": 32
        },
        {
          "ar": "بِالصَّبْرِ",
          "translit": "biṣ-ṣabri",
          "fr": "la patience",
          "racine": "ص-ب-ر",
          "frequence_coran": 103
        }
      ],
      "niveau_couche": 1
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "الْعَصْر",
      "خُسْر",
      "تَوَاصَوْا",
      "الصَّبْر"
    ],
    "racines_a_retenir": [
      "خ-س-ر",
      "ص-ب-ر",
      "و-ص-ي",
      "ح-ق-ق"
    ],
    "declic_frequence": "khusr (racine خ-س-ر, ~65 occurrences) est le vocabulaire du commerce : l'homme est en déficit par défaut, et seuls quatre investissements renversent le bilan — croire, agir, se recommander la vérité, se recommander la patience."
  }
};

export { S103 };
