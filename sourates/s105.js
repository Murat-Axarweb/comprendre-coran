// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S105 = {
  "meta": {
    "numero": 105,
    "nom_ar": "الفيل",
    "nom_translit": "Al-Fil",
    "nom_fr": "L'Éléphant",
    "nom_en": "The Elephant",
    "nom_tr": "Fil",
    "type": "mecquoise",
    "versets_count": 5,
    "theme": "L'armée d'Abraha détruite : Dieu protège Sa Maison sans armée humaine",
    "note_pedagogique": "Événement de l'année de la naissance du Prophète (~570) : l'expédition d'Abraha contre la Kaaba, anéantie. Les Mecquois connaissaient l'histoire — la sourate la transforme en argument.",
    "progression": {
      "couche": 1,
      "prerequis": [
        103
      ],
      "suivant": [
        106
      ]
    },
    "racines_cles": [
      "ف-ع-ل",
      "ك-ي-د",
      "ر-م-ي",
      "أ-ك-ل"
    ],
    "traduction_note": "Traductions pédagogiques originales (FR/EN/TR) — Comprendre le Coran"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "أَلَمْ تَرَ كَيْفَ فَعَلَ رَبُّكَ بِأَصْحَٰبِ الْفِيلِ",
      "translit": "Alam tara kayfa faʿala rabbuka biʾaṣḥābi l-fīl",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "N'as-tu pas vu comment ton Seigneur a agi envers les gens de l'Eléphant"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Have you not considered, [O Muhammad], how your Lord dealt with the companions of the elephant"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Fil sahiplerine Rabbinin ne ettiğini görmedin mi"
        }
      },
      "mots": [
        {
          "ar": "أَلَمْ",
          "translit": "alam",
          "fr": "n'as-tu pas ?",
          "racine": "— (interrogatif)",
          "frequence_coran": 0
        },
        {
          "ar": "تَرَ",
          "translit": "tara",
          "fr": "vu",
          "racine": "ر-أ-ي",
          "frequence_coran": 328
        },
        {
          "ar": "كَيْفَ",
          "translit": "kayfa",
          "fr": "comment",
          "racine": "ك-ي-ف",
          "frequence_coran": 83
        },
        {
          "ar": "فَعَلَ",
          "translit": "faʿala",
          "fr": "a agi",
          "racine": "ف-ع-ل",
          "frequence_coran": 108
        },
        {
          "ar": "رَبُّكَ",
          "translit": "rabbuka",
          "fr": "ton Seigneur",
          "racine": "ر-ب-ب",
          "frequence_coran": 970
        },
        {
          "ar": "بِأَصْحَٰبِ",
          "translit": "biʾaṣḥābi",
          "fr": "envers les gens (de)",
          "racine": "ص-ح-ب",
          "frequence_coran": 97
        },
        {
          "ar": "الْفِيلِ",
          "translit": "l-fīli",
          "fr": "l'Éléphant",
          "racine": "ف-ي-ل",
          "frequence_coran": 1
        }
      ],
      "analyse_globale": "'A-lam tara' (n'as-tu pas vu ?) s'adresse au Prophète qui n'était pas né lors de l'événement : voir, ici, c'est constater par la mémoire collective encore vivante. Le Coran mobilise un fait historique vérifiable par les auditeurs comme preuve de la protection divine.",
      "niveau_couche": 1
    },
    {
      "numero": 2,
      "ar": "أَلَمْ يَجْعَلْ كَيْدَهُمْ فِي تَضْلِيلٍ",
      "translit": "Alam yajʿal kaydahum fī taḍlīl",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "N'a-t-Il pas rendu leur ruse complètement vaine"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Did He not make their plan into misguidance"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onların düzenlerini boşa çıkarmadı mı"
        }
      },
      "mots": [
        {
          "ar": "أَلَمْ",
          "translit": "alam",
          "fr": "n'a-t-Il pas ?",
          "racine": "— (interrogatif)",
          "frequence_coran": 0
        },
        {
          "ar": "يَجْعَلْ",
          "translit": "yajʿal",
          "fr": "fait / rendu",
          "racine": "ج-ع-ل",
          "frequence_coran": 346
        },
        {
          "ar": "كَيْدَهُمْ",
          "translit": "kaydahum",
          "fr": "leur stratagème",
          "racine": "ك-ي-د",
          "frequence_coran": 35
        },
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "en",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "تَضْلِيلٍ",
          "translit": "taḍlīlin",
          "fr": "pure perte / égarement",
          "racine": "ض-ل-ل",
          "frequence_coran": 191
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 3,
      "ar": "وَأَرْسَلَ عَلَيْهِمْ طَيْرًا أَبَابِيلَ",
      "translit": "Waʾarsala ʿalayhim ṭayran abābīl",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et envoyé sur eux des oiseaux par volées"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And He sent against them birds in flocks"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onların üzerine, sert taşlar atan sürülerle kuşlar gönderdi"
        }
      },
      "mots": [
        {
          "ar": "وَأَرْسَلَ",
          "translit": "waʾarsala",
          "fr": "et Il a envoyé",
          "racine": "ر-س-ل",
          "frequence_coran": 513
        },
        {
          "ar": "عَلَيْهِمْ",
          "translit": "ʿalayhim",
          "fr": "contre eux",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "طَيْرًا",
          "translit": "ṭayran",
          "fr": "des oiseaux",
          "racine": "ط-ي-ر",
          "frequence_coran": 24
        },
        {
          "ar": "أَبَابِيلَ",
          "translit": "abābīla",
          "fr": "par nuées successives",
          "racine": "— (nom)",
          "frequence_coran": 1
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 3,
          "fin": 4,
          "principal": true
        }
      }
    },
    {
      "numero": 4,
      "ar": "تَرْمِيهِم بِحِجَارَةٍ مِّن سِجِّيلٍ",
      "translit": "Tarmīhim biḥijāratin min sijjīl",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "qui leur lançaient des pierres d'argile"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Striking them with stones of hard clay"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onların üzerine, sert taşlar atan sürülerle kuşlar gönderdi"
        }
      },
      "mots": [
        {
          "ar": "تَرْمِيهِم",
          "translit": "tarmīhim",
          "fr": "qui les lapidaient",
          "racine": "ر-م-ي",
          "frequence_coran": 9
        },
        {
          "ar": "بِحِجَارَةٍ",
          "translit": "biḥijāratin",
          "fr": "avec des pierres",
          "racine": "ح-ج-ر",
          "frequence_coran": 21
        },
        {
          "ar": "مِّن",
          "translit": "min",
          "fr": "de",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "سِجِّيلٍ",
          "translit": "sijjīlin",
          "fr": "argile durcie",
          "racine": "— (emprunt)",
          "frequence_coran": 3
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 3,
          "fin": 4,
          "principal": false
        }
      }
    },
    {
      "numero": 5,
      "ar": "فَجَعَلَهُمْ كَعَصْفٍ مَّأْكُولٍ",
      "translit": "Fajaʿalahum kaʿaṣfin maʾkūl",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Et Il les a rendus semblables à une paille mâchée"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And He made them like eaten straw"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Sonunda onları, yenilmiş ekin gibi yaptı"
        }
      },
      "mots": [
        {
          "ar": "فَجَعَلَهُمْ",
          "translit": "fajaʿalahum",
          "fr": "et Il les a rendus",
          "racine": "ج-ع-ل",
          "frequence_coran": 346
        },
        {
          "ar": "كَعَصْفٍ",
          "translit": "kaʿaṣfin",
          "fr": "comme un champ",
          "racine": "ع-ص-ف",
          "frequence_coran": 5
        },
        {
          "ar": "مَّأْكُولٍ",
          "translit": "maʾkūli",
          "fr": "dévoré",
          "racine": "أ-ك-ل",
          "frequence_coran": 109
        }
      ],
      "niveau_couche": 1
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "الْفِيل",
      "كَيْد",
      "طَيْرًا",
      "سِجِّيل"
    ],
    "racines_a_retenir": [
      "ك-ي-د",
      "ر-م-ي",
      "ض-ل-ل"
    ],
    "declic_frequence": "kayd (stratagème, ~35 occurrences) traverse le Coran : le complot des puissants paraît irrésistible — frères de Joseph, Pharaon, Abraha — et Dieu le fait toujours 'partir en égarement' (fī tadlil)."
  }
};

export { S105 };
