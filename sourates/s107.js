// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S107 = {
  "meta": {
    "numero": 107,
    "nom_ar": "الماعون",
    "nom_translit": "Al-Ma'un",
    "nom_fr": "L'Aide élémentaire",
    "nom_en": "Small Kindnesses",
    "nom_tr": "Mâûn (Yardımlaşma)",
    "type": "mecquoise",
    "versets_count": 7,
    "theme": "La religion démentie par les actes : repousser l'orphelin, négliger la prière, refuser l'entraide",
    "note_pedagogique": "La sourate lie ce qu'on sépare souvent : le rite (la prière) et le social (l'orphelin, le pauvre, le prêt d'ustensiles). Démentir le Jugement, ce n'est pas d'abord une opinion — c'est un comportement.",
    "progression": {
      "couche": 1,
      "prerequis": [
        104
      ],
      "suivant": [
        109
      ]
    },
    "racines_cles": [
      "ك-ذ-ب",
      "د-ع-ع",
      "ي-ت-م",
      "س-ه-و",
      "ر-أ-ي"
    ],
    "traduction_note": "Traductions pédagogiques originales (FR/EN/TR) — Comprendre le Coran"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "أَرَءَيْتَ الَّذِي يُكَذِّبُ بِالدِّينِ",
      "translit": "Araʾayta lladhī yukadhdhibu bid-dīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Vois-tu celui qui traite de mensonge la Rétribution"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Have you seen the one who denies the Recompense"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Dini yalan sayanı gördün mü"
        }
      },
      "mots": [
        {
          "ar": "أَرَءَيْتَ",
          "translit": "araʾayta",
          "fr": "as-tu vu ?",
          "racine": "ر-أ-ي",
          "frequence_coran": 328
        },
        {
          "ar": "الَّذِي",
          "translit": "lladhī",
          "fr": "celui qui",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "يُكَذِّبُ",
          "translit": "yukadhdhibu",
          "fr": "dément / traite de mensonge",
          "racine": "ك-ذ-ب",
          "frequence_coran": 250
        },
        {
          "ar": "بِالدِّينِ",
          "translit": "bid-dīni",
          "fr": "la Rétribution",
          "racine": "د-ي-ن",
          "frequence_coran": 92
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 2,
      "ar": "فَذَٰلِكَ الَّذِي يَدُعُّ الْيَتِيمَ",
      "translit": "Fadhālika lladhī yaduʿʿu l-yatīm",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "C'est bien lui qui repousse l'orphelin"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "For that is the one who drives away the orphan"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Öksüzü kakıştıran, yoksulu doyurmaya yanaşmayan kimse işte odur"
        }
      },
      "mots": [
        {
          "ar": "فَذَٰلِكَ",
          "translit": "fadhālika",
          "fr": "c'est donc celui-là",
          "racine": "— (démonstratif)",
          "frequence_coran": 0
        },
        {
          "ar": "الَّذِي",
          "translit": "lladhī",
          "fr": "qui",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "يَدُعُّ",
          "translit": "yaduʿʿu",
          "fr": "repousse brutalement",
          "racine": "د-ع-ع",
          "frequence_coran": 2
        },
        {
          "ar": "الْيَتِيمَ",
          "translit": "l-yatīma",
          "fr": "l'orphelin",
          "racine": "ي-ت-م",
          "frequence_coran": 23
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 2,
          "fin": 3,
          "principal": true
        }
      }
    },
    {
      "numero": 3,
      "ar": "وَلَا يَحُضُّ عَلَىٰ طَعَامِ الْمِسْكِينِ",
      "translit": "Walā yaḥuḍḍu ʿalā ṭaʿāmi l-miskīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et qui n'encourage point à nourrir le pauvre"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And does not encourage the feeding of the poor"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Öksüzü kakıştıran, yoksulu doyurmaya yanaşmayan kimse işte odur"
        }
      },
      "mots": [
        {
          "ar": "وَلَا",
          "translit": "walā",
          "fr": "et ne",
          "racine": "— (négation)",
          "frequence_coran": 0
        },
        {
          "ar": "يَحُضُّ",
          "translit": "yaḥuḍḍu",
          "fr": "encourage pas",
          "racine": "ح-ض-ض",
          "frequence_coran": 3
        },
        {
          "ar": "عَلَىٰ",
          "translit": "ʿalā",
          "fr": "à",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "طَعَامِ",
          "translit": "ṭaʿāmi",
          "fr": "nourrir",
          "racine": "ط-ع-م",
          "frequence_coran": 48
        },
        {
          "ar": "الْمِسْكِينِ",
          "translit": "l-miskīni",
          "fr": "le pauvre",
          "racine": "س-ك-ن",
          "frequence_coran": 23
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 2,
          "fin": 3,
          "principal": false
        }
      }
    },
    {
      "numero": 4,
      "ar": "فَوَيْلٌ لِّلْمُصَلِّينَ",
      "translit": "Fawaylun lilmuṣallīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Malheur donc, à ceux qui prient"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "So woe to those who pray"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Vay o namaz kılanların haline ki"
        }
      },
      "mots": [
        {
          "ar": "فَوَيْلٌ",
          "translit": "fawaylun",
          "fr": "malheur donc",
          "racine": "و-ي-ل",
          "frequence_coran": 40
        },
        {
          "ar": "لِّلْمُصَلِّينَ",
          "translit": "lilmuṣallīna",
          "fr": "à ceux qui prient",
          "racine": "ص-ل-و",
          "frequence_coran": 99
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 5,
      "ar": "الَّذِينَ هُمْ عَن صَلَاتِهِمْ سَاهُونَ",
      "translit": "Alladhīna hum ʿan ṣalātihim sāhūn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "tout en négligeant (et retardant) leur Salât"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "[But] who are heedless of their prayer"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onlar kıldıkları namazdan gafildirler"
        }
      },
      "mots": [
        {
          "ar": "الَّذِينَ",
          "translit": "alladhīna",
          "fr": "ceux qui",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "هُمْ",
          "translit": "hum",
          "fr": "eux",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "عَن",
          "translit": "ʿan",
          "fr": "de",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "صَلَاتِهِمْ",
          "translit": "ṣalātihim",
          "fr": "leur prière",
          "racine": "ص-ل-و",
          "frequence_coran": 99
        },
        {
          "ar": "سَاهُونَ",
          "translit": "sāhūna",
          "fr": "sont négligents",
          "racine": "س-ه-و",
          "frequence_coran": 2,
          "famille": [
            "سَهَا (sahā) — être distrait",
            "سَاهٍ (sāhin) — négligent"
          ],
          "profondeur": "Sāhūn : le reproche ne vise pas ceux qui se trompent DANS la prière (fī ṣalātihim) mais ceux qui sont négligents DE leur prière (ʿan ṣalātihim) — ils la retardent, la vident de sens, la font par habitude ou pour être vus. La préposition ʿan change tout : c'est le rapport à la prière qui est jugé, pas les erreurs techniques."
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 6,
      "ar": "الَّذِينَ هُمْ يُرَاءُونَ",
      "translit": "Alladhīna hum yurāʾūn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "qui sont pleins d'ostentation"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Those who make show [of their deeds]"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onlar gösteriş yaparlar"
        }
      },
      "mots": [
        {
          "ar": "الَّذِينَ",
          "translit": "alladhīna",
          "fr": "ceux qui",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "هُمْ",
          "translit": "hum",
          "fr": "eux",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "يُرَاءُونَ",
          "translit": "yurāʾūna",
          "fr": "font pour être vus",
          "racine": "ر-أ-ي",
          "frequence_coran": 328
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 7,
      "ar": "وَيَمْنَعُونَ الْمَاعُونَ",
      "translit": "Wayamnaʿūna l-māʿūn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et refusent l'ustensile (à celui qui en a besoin)"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And withhold [simple] assistance"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onlar basit şeyleri dahi vermezler"
        }
      },
      "mots": [
        {
          "ar": "وَيَمْنَعُونَ",
          "translit": "wayamnaʿūna",
          "fr": "et refusent",
          "racine": "م-ن-ع",
          "frequence_coran": 17
        },
        {
          "ar": "الْمَاعُونَ",
          "translit": "l-māʿūna",
          "fr": "l'aide élémentaire",
          "racine": "م-ع-ن",
          "frequence_coran": 1
        }
      ],
      "analyse_globale": "Al-māʿūn : les petites choses qu'on prête entre voisins — le sel, l'eau, l'outil. Refuser cela révèle un cœur fermé. La sourate a commencé par le déni du Jugement et finit par le refus de prêter une casserole : entre les deux, une seule et même maladie du cœur.",
      "niveau_couche": 1
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "يُكَذِّبُ",
      "الْيَتِيم",
      "الْمِسْكِين",
      "سَاهُون",
      "الْمَاعُون"
    ],
    "racines_a_retenir": [
      "ك-ذ-ب",
      "ص-ل-و",
      "م-ن-ع"
    ],
    "declic_frequence": "kadhdhaba (traiter de mensonge, ~250 occurrences pour la racine ك-ذ-ب) est le verbe le plus fréquent pour décrire le rejet de la foi — et cette sourate montre qu'on peut 'démentir la Rétribution' par ses gestes quotidiens sans jamais le dire à voix haute."
  }
};

export { S107 };
