// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S098 = {
  "meta": {
    "numero": 98,
    "nom_ar": "البينة",
    "nom_translit": "Al-Bayyinah",
    "nom_fr": "La Preuve évidente",
    "nom_en": "The Clear Evidence",
    "nom_tr": "Beyyine (Apaçık Delil)",
    "type": "medinoise",
    "versets_count": 8,
    "theme": "La preuve venue trancher les divisions : le culte pur, la prière, la zakat — la religion de la droiture",
    "note_pedagogique": "Sourate médinoise qui s'adresse aux Gens du Livre et aux associateurs : la division n'est pas venue d'un manque de preuves, mais après leur venue. Elle définit la 'religion de droiture' en trois points d'une simplicité désarmante.",
    "progression": {
      "couche": 2,
      "prerequis": [
        97
      ],
      "suivant": [
        99
      ]
    },
    "racines_cles": [
      "ب-ي-ن",
      "خ-ل-ص",
      "ق-و-م",
      "خ-ي-ر",
      "ر-ض-و"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "لَمْ يَكُنِ الَّذِينَ كَفَرُواْ مِنْ أَهْلِ الْكِتَٰبِ وَالْمُشْرِكِينَ مُنفَكِّينَ حَتَّىٰ تَأْتِيَهُمُ الْبَيِّنَةُ",
      "translit": "Lam yakuni lladhīna kafarū min ahli l-kitābi wal-mushrikīna munfakkīna ḥattā taʾtiyahumu l-bayyinat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Les infidèles parmi les gens du Livre, ainsi que les Associateurs, ne cesseront pas de mécroire jusqu'à ce que leur vienne la Preuve évidente"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Those who disbelieved among the People of the Scripture and the polytheists were not to be parted [from misbelief] until there came to them clear evidence"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kitap ehlinden ve ortak koşanlardan inkarcılar, kendilerine apaçık bir belge, içinde kesin ve en doğru hükümlerin bulunduğu arınmış sahifeleri okuyan, Allah katından bir Peygamber gelene kadar dinlerinden vazgeçecek değillerdi"
        }
      },
      "mots": [
        {
          "ar": "لَمْ",
          "translit": "lam",
          "fr": "ne... pas",
          "en": "Not",
          "tr": "değillerdi",
          "racine": "particule de négation",
          "frequence_coran": 163
        },
        {
          "ar": "يَكُنِ",
          "translit": "yakuni",
          "fr": "ont (pas) été",
          "en": "were",
          "tr": "",
          "racine": "ك-و-ن",
          "frequence_coran": 1360,
          "src": "wbw"
        },
        {
          "ar": "الَّذِينَ",
          "translit": "lladhīna",
          "fr": "ceux qui",
          "en": "those who",
          "tr": "kimseler",
          "racine": "pronom relatif",
          "frequence_coran": 810
        },
        {
          "ar": "كَفَرُواْ",
          "translit": "kafarū",
          "fr": "ils ont mécru",
          "en": "disbelieved",
          "tr": "inkar eden(ler)",
          "racine": "ك-ف-ر",
          "frequence_coran": 189
        },
        {
          "ar": "مِنْ",
          "translit": "min",
          "fr": "de",
          "en": "from",
          "tr": "ehlinden",
          "racine": "particule",
          "frequence_coran": 3226
        },
        {
          "ar": "أَهْلِ",
          "translit": "ahli",
          "fr": "(les) gens",
          "en": "(the) People",
          "tr": "",
          "racine": "ا-ه-ل",
          "frequence_coran": 127,
          "src": "wbw"
        },
        {
          "ar": "الْكِتَٰبِ",
          "translit": "l-kitābi",
          "fr": "(du) Livre",
          "en": "of the Book",
          "tr": "Kitap",
          "racine": "ك-ت-ب",
          "frequence_coran": 256,
          "src": "wbw"
        },
        {
          "ar": "وَالْمُشْرِكِينَ",
          "translit": "wal-mushrikīna",
          "fr": "les associateurs",
          "en": "and the polytheists",
          "tr": "ve müşrikler(den)",
          "racine": "ش-ر-ك",
          "frequence_coran": 49
        },
        {
          "ar": "مُنفَكِّينَ",
          "translit": "munfakkīna",
          "fr": "des gens qui cessent",
          "en": "to be abandoned",
          "tr": "bırakılacak",
          "src": "wbw"
        },
        {
          "ar": "حَتَّىٰ",
          "translit": "ḥattā",
          "fr": "jusqu’à",
          "en": "until",
          "tr": "dek",
          "racine": "particule de limite",
          "frequence_coran": 142
        },
        {
          "ar": "تَأْتِيَهُمُ",
          "translit": "taʾtiyahumu",
          "fr": "leur vienne",
          "en": "(there) comes to them",
          "tr": "kendilerine gelinceye",
          "racine": "أ-ت-ي",
          "frequence_coran": 90,
          "src": "wbw"
        },
        {
          "ar": "الْبَيِّنَةُ",
          "translit": "l-bayyinatu",
          "fr": "preuve évidente",
          "en": "the clear evidence",
          "tr": "açık kanıt",
          "racine": "ب-ي-ن",
          "frequence_coran": 28
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 3,
          "principal": true
        }
      }
    },
    {
      "numero": 2,
      "ar": "رَسُولٌ مِّنَ اللَّهِ يَتْلُواْ صُحُفًا مُّطَهَّرَةً",
      "translit": "Rasūlun mina llāhi yatlū ṣuḥufan muṭahharat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "un Messager, de la part d'Allah, qui leur récite des feuilles purifiées"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "A Messenger from Allah, reciting purified scriptures"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kitap ehlinden ve ortak koşanlardan inkarcılar, kendilerine apaçık bir belge, içinde kesin ve en doğru hükümlerin bulunduğu arınmış sahifeleri okuyan, Allah katından bir Peygamber gelene kadar dinlerinden vazgeçecek değillerdi"
        }
      },
      "mots": [
        {
          "ar": "رَسُولٌ",
          "translit": "rasūlun",
          "fr": "messager",
          "en": "A Messenger",
          "tr": "bir elçi",
          "racine": "ر-س-ل",
          "frequence_coran": 332
        },
        {
          "ar": "مِّنَ",
          "translit": "mina",
          "fr": "de",
          "en": "from",
          "tr": "tarafından (gönderilen)",
          "racine": "particule",
          "frequence_coran": 3226
        },
        {
          "ar": "اللَّهِ",
          "translit": "llāhi",
          "fr": "Allah",
          "en": "Allah",
          "tr": "Allah",
          "racine": "أ-ل-ه",
          "frequence_coran": 2699
        },
        {
          "ar": "يَتْلُواْ",
          "translit": "yatlū",
          "fr": "(qui) récite",
          "en": "reciting",
          "tr": "okuyan",
          "racine": "ت-ل-و",
          "frequence_coran": 61,
          "src": "wbw"
        },
        {
          "ar": "صُحُفًا",
          "translit": "ṣuḥufan",
          "fr": "des pages",
          "en": "pages",
          "tr": "sahifeler",
          "racine": "ص-ح-ف",
          "frequence_coran": 6,
          "src": "wbw"
        },
        {
          "ar": "مُّطَهَّرَةً",
          "translit": "muṭahharatan",
          "fr": "purifiées,",
          "en": "purified",
          "tr": "tertemiz",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 3,
          "principal": false
        }
      }
    },
    {
      "numero": 3,
      "ar": "فِيهَا كُتُبٌ قَيِّمَةٌ",
      "translit": "Fīhā kutubun qayyimat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "dans lesquelles se trouvent des prescriptions d'une rectitude parfaite"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Within which are correct writings"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kitap ehlinden ve ortak koşanlardan inkarcılar, kendilerine apaçık bir belge, içinde kesin ve en doğru hükümlerin bulunduğu arınmış sahifeleri okuyan, Allah katından bir Peygamber gelene kadar dinlerinden vazgeçecek değillerdi"
        }
      },
      "mots": [
        {
          "ar": "فِيهَا",
          "translit": "fīhā",
          "fr": "en elle",
          "en": "Wherein",
          "tr": "onda vardır (sahifelerde)",
          "racine": "فِي + هَا",
          "frequence_coran": 241
        },
        {
          "ar": "كُتُبٌ",
          "translit": "kutubun",
          "fr": "des Écrits",
          "en": "(are) writings",
          "tr": "Kitaplar",
          "racine": "ك-ت-ب",
          "frequence_coran": 256,
          "src": "wbw"
        },
        {
          "ar": "قَيِّمَةٌ",
          "translit": "qayyimatun",
          "fr": "Corrects.",
          "en": "correct",
          "tr": "doğru değerli'",
          "racine": "ق-و-م",
          "frequence_coran": 70,
          "src": "wbw"
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 3,
          "principal": false
        }
      }
    },
    {
      "numero": 4,
      "ar": "وَمَا تَفَرَّقَ الَّذِينَ أُوتُواْ الْكِتَٰبَ إِلَّا مِن بَعْدِ مَا جَاءَتْهُمُ الْبَيِّنَةُ",
      "translit": "Wamā tafarraqa lladhīna ūtū l-kitāba illā min baʿdi mā jāʾathumu l-bayyinat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Et ceux à qui le Livre a été donné ne se sont divisés qu'après que la preuve leur fut venue"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Nor did those who were given the Scripture become divided until after there had come to them clear evidence"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Ama, kendilerine kitap verilenler, onlara apaçık belge geldikten sonra ayrılığa düştüler"
        }
      },
      "mots": [
        {
          "ar": "وَمَا",
          "translit": "wamā",
          "fr": "et ce que",
          "en": "And not",
          "tr": "ve",
          "racine": "و + مَا",
          "frequence_coran": 646
        },
        {
          "ar": "تَفَرَّقَ",
          "translit": "tafarraqa",
          "fr": "se sont divisés",
          "en": "became divided",
          "tr": "ayrılığa düşmediler",
          "src": "wbw"
        },
        {
          "ar": "الَّذِينَ",
          "translit": "lladhīna",
          "fr": "ceux qui",
          "en": "those who",
          "tr": "olanlar",
          "racine": "pronom relatif",
          "frequence_coran": 810
        },
        {
          "ar": "أُوتُواْ",
          "translit": "ūtū",
          "fr": "ils ont reçu",
          "en": "were given",
          "tr": "verilmiş",
          "racine": "أ-ت-ي",
          "frequence_coran": 47
        },
        {
          "ar": "الْكِتَٰبَ",
          "translit": "l-kitāba",
          "fr": "Le Livre",
          "en": "the Book",
          "tr": "Kitap",
          "racine": "ك-ت-ب",
          "frequence_coran": 256,
          "src": "wbw"
        },
        {
          "ar": "إِلَّا",
          "translit": "illā",
          "fr": "sauf",
          "en": "until",
          "tr": "ancak (düştüler)",
          "racine": "particule d’exception",
          "frequence_coran": 662
        },
        {
          "ar": "مِن",
          "translit": "min",
          "fr": "de",
          "en": "from",
          "tr": "(ondan) sonra",
          "racine": "particule",
          "frequence_coran": 3226
        },
        {
          "ar": "بَعْدِ",
          "translit": "baʿdi",
          "fr": "après",
          "en": "after",
          "tr": "",
          "racine": "ب-ع-د",
          "frequence_coran": 82
        },
        {
          "ar": "مَا",
          "translit": "mā",
          "fr": "ce que",
          "en": "what",
          "tr": "ne ki",
          "racine": "particule",
          "frequence_coran": 1010
        },
        {
          "ar": "جَاءَتْهُمُ",
          "translit": "jāʾathumu",
          "fr": "est arrivée (chez) eux",
          "en": "came (to) them",
          "tr": "kendilerine geldi",
          "src": "wbw"
        },
        {
          "ar": "الْبَيِّنَةُ",
          "translit": "l-bayyinatu",
          "fr": "preuve évidente",
          "en": "(of) the clear evidence",
          "tr": "açık kanıt",
          "racine": "ب-ي-ن",
          "frequence_coran": 28
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 5,
      "ar": "وَمَا أُمِرُواْ إِلَّا لِيَعْبُدُواْ اللَّهَ مُخْلِصِينَ لَهُ الدِّينَ حُنَفَاءَ وَيُقِيمُواْ الصَّلَوٰةَ وَيُؤْتُواْ الزَّكَوٰةَ وَذَٰلِكَ دِينُ الْقَيِّمَةِ",
      "translit": "Wamā umirū illā liyaʿbudū llāha mukhliṣīna lahu d-dīna ḥunafāʾa wayuqīmū ṣ-ṣalawāta wayuʾtū z-zakawāta wadhālika dīnu l-qayyimat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Il ne leur a été commandé, cependant, que d'adorer Allah, Lui vouant un culte exclusif, d'accomplir la Salât et d'acquitter la Zakât. Et voilà la religion de droiture"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And they were not commanded except to worship Allah, [being] sincere to Him in religion, inclining to truth, and to establish prayer and to give zakah. And that is the correct religion"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Oysa onlar, doğruya yönelerek, dini yalnız Allah'a has kılarak O'na kulluk etmek, namazı kılmak ve zekatı vermekle emrolunmuşlardı. Dosdoğru olan din de budur"
        }
      },
      "mots": [
        {
          "ar": "وَمَا",
          "translit": "wamā",
          "fr": "et ce que",
          "en": "And not",
          "tr": "oysa",
          "racine": "و + مَا",
          "frequence_coran": 646
        },
        {
          "ar": "أُمِرُواْ",
          "translit": "umirū",
          "fr": "ils ont reçu l’ordre",
          "en": "they were commanded",
          "tr": "kendilerine emredilmedi",
          "racine": "أ-م-ر",
          "frequence_coran": 166,
          "src": "wbw"
        },
        {
          "ar": "إِلَّا",
          "translit": "illā",
          "fr": "sauf",
          "en": "except",
          "tr": "dışında (bir şey)",
          "racine": "particule d’exception",
          "frequence_coran": 662
        },
        {
          "ar": "لِيَعْبُدُواْ",
          "translit": "liyaʿbudū",
          "fr": "d’adorer",
          "en": "to worship",
          "tr": "kulluk etmeleri",
          "racine": "ع-ب-د",
          "frequence_coran": 40,
          "src": "wbw"
        },
        {
          "ar": "اللَّهَ",
          "translit": "llāha",
          "fr": "Allah",
          "en": "Allah",
          "tr": "Allah'a",
          "racine": "أ-ل-ه",
          "frequence_coran": 2699
        },
        {
          "ar": "مُخْلِصِينَ",
          "translit": "mukhliṣīna",
          "fr": "(en étant) des gens qui rendent exclusive",
          "en": "(being) sincere",
          "tr": "halis kılarak",
          "racine": "خ-ل-ص",
          "frequence_coran": 11,
          "src": "wbw"
        },
        {
          "ar": "لَهُ",
          "translit": "lahu",
          "fr": "à lui",
          "en": "to Him",
          "tr": "O'na",
          "racine": "لِ + هُ",
          "frequence_coran": 275
        },
        {
          "ar": "الدِّينَ",
          "translit": "d-dīna",
          "fr": "religion",
          "en": "(in) the religion",
          "tr": "dini",
          "racine": "د-ي-ن",
          "frequence_coran": 94
        },
        {
          "ar": "حُنَفَاءَ",
          "translit": "ḥunafāʾa",
          "fr": "inclinés à la droiture,",
          "en": "upright",
          "tr": "birleyerek",
          "racine": "ح-ن-ف",
          "frequence_coran": 5,
          "src": "wbw"
        },
        {
          "ar": "وَيُقِيمُواْ",
          "translit": "wayuqīmū",
          "fr": "et (d’)accomplir",
          "en": "and to establish",
          "tr": "ve kılmaları",
          "racine": "ق-و-م",
          "frequence_coran": 21,
          "src": "wbw"
        },
        {
          "ar": "الصَّلَوٰةَ",
          "translit": "ṣ-ṣalawāta",
          "fr": "la prière",
          "en": "the prayer",
          "tr": "namazı",
          "racine": "ص-ل-و",
          "frequence_coran": 82,
          "src": "wbw"
        },
        {
          "ar": "وَيُؤْتُواْ",
          "translit": "wayuʾtū",
          "fr": "et (de) donner",
          "en": "and to give",
          "tr": "ve vermeleri",
          "racine": "أ-ت-ي",
          "frequence_coran": 90,
          "src": "wbw"
        },
        {
          "ar": "الزَّكَوٰةَ",
          "translit": "z-zakawāta",
          "fr": "le don charitable.",
          "en": "the zakah",
          "tr": "zekatı",
          "racine": "ز-ك-و",
          "frequence_coran": 32,
          "src": "wbw"
        },
        {
          "ar": "وَذَٰلِكَ",
          "translit": "wadhālika",
          "fr": "cela",
          "en": "And that",
          "tr": "işte budur",
          "racine": "pronom démonstratif",
          "frequence_coran": 280
        },
        {
          "ar": "دِينُ",
          "translit": "dīnu",
          "fr": "religion",
          "en": "(is the) religion",
          "tr": "din",
          "racine": "د-ي-ن",
          "frequence_coran": 94
        },
        {
          "ar": "الْقَيِّمَةِ",
          "translit": "l-qayyimati",
          "fr": "(de) celle qui est correcte.",
          "en": "the correct",
          "tr": "doğru",
          "racine": "ق-و-م",
          "frequence_coran": 70,
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 6,
      "ar": "إِنَّ الَّذِينَ كَفَرُواْ مِنْ أَهْلِ الْكِتَٰبِ وَالْمُشْرِكِينَ فِي نَارِ جَهَنَّمَ خَٰلِدِينَ فِيهَا أُوْلَـٰئِكَ هُمْ شَرُّ الْبَرِيَّةِ",
      "translit": "Inna lladhīna kafarū min ahli l-kitābi wal-mushrikīna fī nāri jahannama khālidīna fīhā ūlāʾika hum sharru l-bariyyat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Les infidèles parmi les gens du Livre, ainsi que les Associateurs iront au feu de l'Enfer, pour y demeurer éternellement. De toute la création, ce sont eux les pires"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Indeed, they who disbelieved among the People of the Scripture and the polytheists will be in the fire of Hell, abiding eternally therein. Those are the worst of creatures"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kitap ehlinden ve ortak koşanlardan inkar edenler, şüphesiz içinde temelli kalacakları cehennem ateşindedirler. İşte bunlar, yaratıkların en kötüsüdürler"
        }
      },
      "mots": [
        {
          "ar": "إِنَّ",
          "translit": "inna",
          "fr": "certes",
          "en": "Indeed",
          "tr": "şüphesiz",
          "racine": "particule d’insistance",
          "frequence_coran": 609
        },
        {
          "ar": "الَّذِينَ",
          "translit": "lladhīna",
          "fr": "ceux qui",
          "en": "those who",
          "tr": "kimseler",
          "racine": "pronom relatif",
          "frequence_coran": 810
        },
        {
          "ar": "كَفَرُواْ",
          "translit": "kafarū",
          "fr": "ils ont mécru",
          "en": "disbelieve",
          "tr": "inkarcı(lar)",
          "racine": "ك-ف-ر",
          "frequence_coran": 189
        },
        {
          "ar": "مِنْ",
          "translit": "min",
          "fr": "de",
          "en": "from",
          "tr": "ehlinden",
          "racine": "particule",
          "frequence_coran": 3226
        },
        {
          "ar": "أَهْلِ",
          "translit": "ahli",
          "fr": "(les) gens",
          "en": "(the) People",
          "tr": "",
          "racine": "ا-ه-ل",
          "frequence_coran": 127,
          "src": "wbw"
        },
        {
          "ar": "الْكِتَٰبِ",
          "translit": "l-kitābi",
          "fr": "(du) Livre",
          "en": "(of) the Book",
          "tr": "Kitap",
          "racine": "ك-ت-ب",
          "frequence_coran": 256,
          "src": "wbw"
        },
        {
          "ar": "وَالْمُشْرِكِينَ",
          "translit": "wal-mushrikīna",
          "fr": "les associateurs",
          "en": "and the polytheists",
          "tr": "ve ortak koşanlar",
          "racine": "ش-ر-ك",
          "frequence_coran": 49
        },
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "dans",
          "en": "(will be) in",
          "tr": "içindedirler",
          "racine": "particule",
          "frequence_coran": 4381
        },
        {
          "ar": "نَارِ",
          "translit": "nāri",
          "fr": "feu",
          "en": "(the) Fire",
          "tr": "ateşi",
          "racine": "ن-و-ر",
          "frequence_coran": 145
        },
        {
          "ar": "جَهَنَّمَ",
          "translit": "jahannama",
          "fr": "Jahannam",
          "en": "(of) Hell",
          "tr": "cehennem",
          "racine": "nom propre",
          "frequence_coran": 54
        },
        {
          "ar": "خَٰلِدِينَ",
          "translit": "khālidīna",
          "fr": "(étant) éternellement restants",
          "en": "abiding eternally",
          "tr": "sürekli olarak",
          "racine": "خ-ل-د",
          "frequence_coran": 3,
          "src": "wbw"
        },
        {
          "ar": "فِيهَا",
          "translit": "fīhā",
          "fr": "en elle",
          "en": "therein",
          "tr": "orada",
          "racine": "فِي + هَا",
          "frequence_coran": 241
        },
        {
          "ar": "أُوْلَـٰئِكَ",
          "translit": "ūlāʾika",
          "fr": "ceux-là",
          "en": "Those",
          "tr": "işte",
          "racine": "pronom démonstratif",
          "frequence_coran": 133
        },
        {
          "ar": "هُمْ",
          "translit": "hum",
          "fr": "eux",
          "en": "they",
          "tr": "onlar",
          "racine": "pronom personnel",
          "frequence_coran": 180
        },
        {
          "ar": "شَرُّ",
          "translit": "sharru",
          "fr": "mal",
          "en": "(are the) worst",
          "tr": "en şerlisidir",
          "racine": "ش-ر-ر",
          "frequence_coran": 8
        },
        {
          "ar": "الْبَرِيَّةِ",
          "translit": "l-bariyyati",
          "fr": "(des) créatures.",
          "en": "(of) the creatures",
          "tr": "yaratılmışların",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 7,
      "ar": "إِنَّ الَّذِينَ ءَامَنُواْ وَعَمِلُواْ الصَّـٰلِحَٰتِ أُوْلَـٰئِكَ هُمْ خَيْرُ الْبَرِيَّةِ",
      "translit": "Inna lladhīna āmanū waʿamilū ṣ-ṣāliḥāti ūlāʾika hum khayru l-bariyyat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Quant à ceux qui croient et accomplissent les bonnes œuvres, ce sont les meilleurs de toute la création"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Indeed, they who have believed and done righteous deeds - those are the best of creatures"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Fakat, inanıp yararlı iş işleyenler, işte onlar da, yaratıkların en iyileridirler"
        }
      },
      "mots": [
        {
          "ar": "إِنَّ",
          "translit": "inna",
          "fr": "certes",
          "en": "Indeed",
          "tr": "şüphesiz",
          "racine": "particule d’insistance",
          "frequence_coran": 609
        },
        {
          "ar": "الَّذِينَ",
          "translit": "lladhīna",
          "fr": "ceux qui",
          "en": "those who",
          "tr": "kimseler",
          "racine": "pronom relatif",
          "frequence_coran": 810
        },
        {
          "ar": "ءَامَنُواْ",
          "translit": "āmanū",
          "fr": "ont accepté la foi",
          "en": "believe",
          "tr": "inanan(lar)",
          "racine": "ا-م-ن",
          "frequence_coran": 440,
          "src": "wbw"
        },
        {
          "ar": "وَعَمِلُواْ",
          "translit": "waʿamilū",
          "fr": "et ils ont œuvré",
          "en": "and do",
          "tr": "ve yapanlar",
          "racine": "و + ع-م-ل",
          "frequence_coran": 53
        },
        {
          "ar": "الصَّـٰلِحَٰتِ",
          "translit": "ṣ-ṣāliḥāti",
          "fr": "les actions vertueuses,",
          "en": "righteous deeds",
          "tr": "iyi işler",
          "racine": "ص-ل-ح",
          "frequence_coran": 61,
          "src": "wbw"
        },
        {
          "ar": "أُوْلَـٰئِكَ",
          "translit": "ūlāʾika",
          "fr": "ceux-là",
          "en": "those",
          "tr": "işte",
          "racine": "pronom démonstratif",
          "frequence_coran": 133
        },
        {
          "ar": "هُمْ",
          "translit": "hum",
          "fr": "eux",
          "en": "they",
          "tr": "onlar",
          "racine": "pronom personnel",
          "frequence_coran": 180
        },
        {
          "ar": "خَيْرُ",
          "translit": "khayru",
          "fr": "meilleur",
          "en": "(are the) best",
          "tr": "en hayırlısıdır",
          "racine": "خ-ي-ر",
          "frequence_coran": 78
        },
        {
          "ar": "الْبَرِيَّةِ",
          "translit": "l-bariyyati",
          "fr": "(des) créatures.",
          "en": "(of) the creatures",
          "tr": "yaratılmışların",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 8,
      "ar": "جَزَاؤُهُمْ عِندَ رَبِّهِمْ جَنَّـٰتُ عَدْنٍ تَجْرِي مِن تَحْتِهَا الْأَنْهَٰرُ خَٰلِدِينَ فِيهَا أَبَدًا رَّضِيَ اللَّهُ عَنْهُمْ وَرَضُواْ عَنْهُ ذَٰلِكَ لِمَنْ خَشِيَ رَبَّهُ",
      "translit": "Jazāʾuhum ʿinda rabbihim jannātu ʿadnin tajrī min taḥtihā l-ʾanhāru khālidīna fīhā abadan raḍiya llāhu ʿanhum waraḍū ʿanhu dhālika liman khashiya rabbahū",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Leur récompense auprès d'Allah sera les Jardins de séjour, sous lesquels coulent les ruisseaux, pour y demeurer éternellement. Allah les agrée et ils L'agréent. Telle sera [la récompense] de celui qui craint son Seigneur"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Their reward with Allah will be gardens of perpetual residence beneath which rivers flow, wherein they will abide forever, Allah being pleased with them and they with Him. That is for whoever has feared his Lord"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onların Rableri katındaki mükafatı, içinde temelli ve sonsuz kalacakları, içlerinden ırmaklar akan Adn cennetleridir. Allah onlardan razıdır. Onlar da Allah'tan razıdır. Bu, Rabbinden korkan kimseyedir"
        }
      },
      "mots": [
        {
          "ar": "جَزَاؤُهُمْ",
          "translit": "jazāʾuhum",
          "fr": "Leur récompense non-diminuée",
          "en": "Their reward",
          "tr": "onların mükafatı",
          "racine": "ج-ز-ي",
          "frequence_coran": 42,
          "src": "wbw"
        },
        {
          "ar": "عِندَ",
          "translit": "ʿinda",
          "fr": "auprès de",
          "en": "(is) with",
          "tr": "katında",
          "racine": "adverbe / préposition",
          "frequence_coran": 98
        },
        {
          "ar": "رَبِّهِمْ",
          "translit": "rabbihim",
          "fr": "leur Seigneur",
          "en": "their Lord",
          "tr": "Rableri",
          "racine": "ر-ب-ب",
          "frequence_coran": 80
        },
        {
          "ar": "جَنَّـٰتُ",
          "translit": "jannātu",
          "fr": "des jardins à hautes herbes",
          "en": "Gardens",
          "tr": "cennetleridir",
          "racine": "ج-ن-ن",
          "frequence_coran": 147,
          "src": "wbw"
        },
        {
          "ar": "عَدْنٍ",
          "translit": "ʿadnin",
          "fr": "Éden (séjour éternel)",
          "en": "(of) Eternity",
          "tr": "Adn",
          "racine": "ع-د-ن",
          "frequence_coran": 11
        },
        {
          "ar": "تَجْرِي",
          "translit": "tajrī",
          "fr": "elle coule",
          "en": "flow",
          "tr": "akan",
          "racine": "ج-ر-ي",
          "frequence_coran": 48
        },
        {
          "ar": "مِن",
          "translit": "min",
          "fr": "de",
          "en": "from",
          "tr": "altlarından",
          "racine": "particule",
          "frequence_coran": 3226
        },
        {
          "ar": "تَحْتِهَا",
          "translit": "taḥtihā",
          "fr": "sous eux",
          "en": "underneath them",
          "tr": "",
          "racine": "ت-ح-ت",
          "frequence_coran": 51,
          "src": "wbw"
        },
        {
          "ar": "الْأَنْهَٰرُ",
          "translit": "l-ʾanhāru",
          "fr": "les rivières,",
          "en": "the rivers",
          "tr": "ırmaklar",
          "racine": "ن-ه-ر",
          "frequence_coran": 54,
          "src": "wbw"
        },
        {
          "ar": "خَٰلِدِينَ",
          "translit": "khālidīna",
          "fr": "(étant) éternellement restants",
          "en": "will abide",
          "tr": "sürekli kalacakları",
          "racine": "خ-ل-د",
          "frequence_coran": 3,
          "src": "wbw"
        },
        {
          "ar": "فِيهَا",
          "translit": "fīhā",
          "fr": "en elle",
          "en": "therein",
          "tr": "içinde",
          "racine": "فِي + هَا",
          "frequence_coran": 241
        },
        {
          "ar": "أَبَدًا",
          "translit": "abadan",
          "fr": "à jamais",
          "en": "forever",
          "tr": "ebedi",
          "racine": "أ-ب-د",
          "frequence_coran": 44
        },
        {
          "ar": "رَّضِيَ",
          "translit": "raḍiya",
          "fr": "agréer",
          "en": "(will be) pleased",
          "tr": "razı olmuştur",
          "racine": "ر-ض-و",
          "frequence_coran": 57
        },
        {
          "ar": "اللَّهُ",
          "translit": "llāhu",
          "fr": "Allah",
          "en": "Allah",
          "tr": "Allah",
          "racine": "أ-ل-ه",
          "frequence_coran": 2699
        },
        {
          "ar": "عَنْهُمْ",
          "translit": "ʿanhum",
          "fr": "d’eux",
          "en": "with them",
          "tr": "onlardan",
          "racine": "عَنْ + هُمْ",
          "frequence_coran": 67
        },
        {
          "ar": "وَرَضُواْ",
          "translit": "waraḍū",
          "fr": "et ils sont devenus satisfaits",
          "en": "and they (will be) pleased",
          "tr": "onlar da razı olmuşlardır",
          "racine": "ر-ض-و",
          "frequence_coran": 57,
          "src": "wbw"
        },
        {
          "ar": "عَنْهُ",
          "translit": "ʿanhu",
          "fr": "de Lui.",
          "en": "with Him",
          "tr": "O'ndan",
          "racine": "préposition",
          "frequence_coran": 153,
          "src": "wbw"
        },
        {
          "ar": "ذَٰلِكَ",
          "translit": "dhālika",
          "fr": "cela",
          "en": "That",
          "tr": "bu",
          "racine": "pronom démonstratif",
          "frequence_coran": 280
        },
        {
          "ar": "لِمَنْ",
          "translit": "liman",
          "fr": "pour celui qui",
          "en": "(is) for whoever",
          "tr": "mahsustur",
          "racine": "لِ + مَنْ",
          "frequence_coran": 55
        },
        {
          "ar": "خَشِيَ",
          "translit": "khashiya",
          "fr": "redouter (avec révérence)",
          "en": "feared",
          "tr": "saygı gösterene",
          "racine": "خ-ش-ي",
          "frequence_coran": 48
        },
        {
          "ar": "رَبَّهُ",
          "translit": "rabbahū",
          "fr": "son Seigneur",
          "en": "his Lord",
          "tr": "Rabbine",
          "racine": "ر-ب-ب",
          "frequence_coran": 44
        }
      ],
      "niveau_couche": 2
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "الْبَيِّنَة",
      "مُخْلِصِين",
      "دِين الْقَيِّمَة",
      "خَيْر الْبَرِيَّة",
      "رَضِيَ"
    ],
    "racines_a_retenir": [
      "ب-ي-ن",
      "خ-ل-ص",
      "ر-ض-و"
    ],
    "declic_frequence": "bayyina (racine ب-ي-ن, ~523 occurrences) : la 'preuve claire' n'est pas un théorème mais un messager porteur de feuillets purifiés. Dans le Coran, la clarté ultime a un visage et une voix — la vérité se rencontre plus qu'elle ne se démontre."
  }
};

export { S098 };
