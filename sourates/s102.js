// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S102 = {
  "meta": {
    "numero": 102,
    "nom_ar": "التكاثر",
    "nom_translit": "At-Takathur",
    "nom_fr": "La Course aux richesses",
    "nom_en": "Rivalry in Increase",
    "nom_tr": "Tekâsür (Çoğaltma Yarışı)",
    "type": "mecquoise",
    "versets_count": 8,
    "theme": "La course à l'accumulation qui distrait jusqu'au cimetière — et la triple annonce du savoir certain",
    "note_pedagogique": "Alhākum at-takāthur : deux mots qui radiographient toute société de consommation. La sourate ne condamne pas la richesse mais la course (takāthur, forme de rivalité) qui 'distrait' (alhā) de l'essentiel — jusqu'à ce que la visite au cimetière tranche.",
    "progression": {
      "couche": 2,
      "prerequis": [
        101
      ],
      "suivant": [
        103
      ]
    },
    "racines_cles": [
      "ل-ه-و",
      "ك-ث-ر",
      "ز-و-ر",
      "ع-ل-م",
      "ن-ع-م"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "أَلْهَىٰكُمُ التَّكَاثُرُ",
      "translit": "Alhākumu t-takāthur",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "La course aux richesses vous distrait"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Competition in [worldly] increase diverts you"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Çoğunluk olmak iddianız sizi o kadar meşgul etti ki, mezarları ziyaretle oradakileri de sayacak kadar oldunuz"
        }
      },
      "mots": [],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 2,
          "principal": true
        }
      }
    },
    {
      "numero": 2,
      "ar": "حَتَّىٰ زُرْتُمُ الْمَقَابِرَ",
      "translit": "Ḥattā zurtumu l-maqābir",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "jusqu'à ce que vous visitiez les tombes"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Until you visit the graveyards"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Çoğunluk olmak iddianız sizi o kadar meşgul etti ki, mezarları ziyaretle oradakileri de sayacak kadar oldunuz"
        }
      },
      "mots": [
        {
          "ar": "حَتَّىٰ",
          "translit": "ḥattā",
          "fr": "jusqu’à",
          "racine": "particule de limite",
          "frequence_coran": 142
        }
      ],
      "analyse_globale": "Ḥattā zurtum al-maqābir : 'jusqu'à ce que vous VISITIEZ les cimetières' — le verbe zāra (visiter) est saisissant : même mort, l'homme n'est qu'en visite au cimetière, en transit vers autre chose. Certains commentateurs y lisent aussi l'ironie : la course dure littéralement jusqu'à la tombe.",
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 2,
          "principal": false
        }
      }
    },
    {
      "numero": 3,
      "ar": "كَلَّا سَوْفَ تَعْلَمُونَ",
      "translit": "Kallā sawfa taʿlamūn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Mais non! Vous saurez bientôt"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "No! You are going to know"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Hayır; öyle olmayın; yakında bileceksiniz"
        }
      },
      "mots": [
        {
          "ar": "كَلَّا",
          "translit": "kallā",
          "fr": "que non !",
          "racine": "—",
          "frequence_coran": 8
        },
        {
          "ar": "سَوْفَ",
          "translit": "sawfa",
          "fr": "particule du futur (bientôt",
          "racine": "—",
          "frequence_coran": 42
        },
        {
          "ar": "تَعْلَمُونَ",
          "translit": "taʿlamūna",
          "fr": "vous savez",
          "racine": "ع-ل-م",
          "frequence_coran": 55
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 4,
      "ar": "ثُمَّ كَلَّا سَوْفَ تَعْلَمُونَ",
      "translit": "Thumma kallā sawfa taʿlamūn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "(Encore une fois)! Vous saurez bientôt"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Then no! You are going to know"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Hayır; gözünüzü açın; yakında bileceksiniz"
        }
      },
      "mots": [
        {
          "ar": "ثُمَّ",
          "translit": "thumma",
          "fr": "puis",
          "racine": "particule de succession",
          "frequence_coran": 337
        },
        {
          "ar": "كَلَّا",
          "translit": "kallā",
          "fr": "que non !",
          "racine": "—",
          "frequence_coran": 8
        },
        {
          "ar": "سَوْفَ",
          "translit": "sawfa",
          "fr": "particule du futur (bientôt",
          "racine": "—",
          "frequence_coran": 42
        },
        {
          "ar": "تَعْلَمُونَ",
          "translit": "taʿlamūna",
          "fr": "vous savez",
          "racine": "ع-ل-م",
          "frequence_coran": 55
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 5,
      "ar": "كَلَّا لَوْ تَعْلَمُونَ عِلْمَ الْيَقِينِ",
      "translit": "Kallā law taʿlamūna ʿilma l-yaqīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Sûrement! Si vous saviez de science certaine"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "No! If you only knew with knowledge of certainty"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Dikkat edin, şayet yaptığınızın sonucunu kesin olarak bir bilseniz"
        }
      },
      "mots": [
        {
          "ar": "كَلَّا",
          "translit": "kallā",
          "fr": "que non !",
          "racine": "—",
          "frequence_coran": 8
        },
        {
          "ar": "لَوْ",
          "translit": "law",
          "fr": "si",
          "racine": "particule conditionnelle",
          "frequence_coran": 77
        },
        {
          "ar": "تَعْلَمُونَ",
          "translit": "taʿlamūna",
          "fr": "vous savez",
          "racine": "ع-ل-م",
          "frequence_coran": 55
        },
        {
          "ar": "عِلْمَ",
          "translit": "ʿilma",
          "fr": "savoir",
          "racine": "ع-ل-م",
          "frequence_coran": 105
        },
        {
          "ar": "الْيَقِينِ",
          "translit": "l-yaqīni",
          "fr": "certitude",
          "racine": "ي-ق-ن",
          "frequence_coran": 8
        }
      ],
      "analyse_globale": "ʿIlm al-yaqīn puis ʿayn al-yaqīn (v.7) : le Coran distingue les degrés de certitude — savoir par preuve (ʿilm), voir de ses yeux (ʿayn), puis la tradition ajoutera ḥaqq al-yaqīn, vivre la chose. La sourate fait monter le lecteur d'un degré à l'autre.",
      "niveau_couche": 2
    },
    {
      "numero": 6,
      "ar": "لَتَرَوُنَّ الْجَحِيمَ",
      "translit": "Latarawunna l-jaḥīm",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Vous verrez, certes, la Fournaise"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "You will surely see the Hellfire"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "And olsun ki, cehennemi göreceksiniz"
        }
      },
      "mots": [
        {
          "ar": "الْجَحِيمَ",
          "translit": "l-jaḥīma",
          "fr": "la Fournaise",
          "racine": "ج-ح-م",
          "frequence_coran": 43
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 7,
      "ar": "ثُمَّ لَتَرَوُنَّهَا عَيْنَ الْيَقِينِ",
      "translit": "Thumma latarawunnahā ʿayna l-yaqīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Puis, vous la verrez certes, avec l'œil de la certitude"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Then you will surely see it with the eye of certainty"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "And olsun ki, onu gözünüzle kesin olarak göreceksiniz"
        }
      },
      "mots": [
        {
          "ar": "ثُمَّ",
          "translit": "thumma",
          "fr": "puis",
          "racine": "particule de succession",
          "frequence_coran": 337
        },
        {
          "ar": "عَيْنَ",
          "translit": "ʿayna",
          "fr": "œil",
          "racine": "ع-ي-ن",
          "frequence_coran": 23
        },
        {
          "ar": "الْيَقِينِ",
          "translit": "l-yaqīni",
          "fr": "certitude",
          "racine": "ي-ق-ن",
          "frequence_coran": 8
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 8,
      "ar": "ثُمَّ لَتُسْـءَلُنَّ يَوْمَئِذٍ عَنِ النَّعِيمِ",
      "translit": "Thumma latusʾalunna yawmaʾidhin ʿani n-naʿīm",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Puis, assurément, vous serez interrogés, ce jour-là, sur les délices"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Then you will surely be asked that Day about pleasure"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Sonra o gün, size verilmiş olan her nimetten sorguya çekileceksiniz"
        }
      },
      "mots": [
        {
          "ar": "ثُمَّ",
          "translit": "thumma",
          "fr": "puis",
          "racine": "particule de succession",
          "frequence_coran": 337
        },
        {
          "ar": "يَوْمَئِذٍ",
          "translit": "yawmaʾidhin",
          "fr": "ce jour-là",
          "racine": "ي-و-م + إِذْ",
          "frequence_coran": 63
        },
        {
          "ar": "عَنِ",
          "translit": "ʿani",
          "fr": "de",
          "racine": "préposition",
          "frequence_coran": 153
        },
        {
          "ar": "النَّعِيمِ",
          "translit": "n-naʿīmi",
          "fr": "délices (du Paradis)",
          "racine": "ن-ع-م",
          "frequence_coran": 17
        }
      ],
      "analyse_globale": "An-naʿīm : les délices sur lesquels on sera interrogé — l'eau fraîche, la sécurité, la santé, dit le hadith. Le dernier mot de la sourate retourne la course initiale : ce que vous accumuliez sans compter vous sera compté, bienfait par bienfait.",
      "niveau_couche": 2
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "أَلْهَىٰكُم",
      "التَّكَاثُر",
      "الْمَقَابِر",
      "عِلْم الْيَقِين",
      "النَّعِيم"
    ],
    "racines_a_retenir": [
      "ل-ه-و",
      "ك-ث-ر",
      "ي-ق-ن",
      "ن-ع-م"
    ],
    "declic_frequence": "takāthur : forme tafāʿul de kathura (être nombreux) — la forme grammaticale de la rivalité mutuelle. Ce n'est pas 'avoir beaucoup' qui est visé, mais 'avoir plus que l'autre' : la comparaison comme moteur, l'accumulation comme compétition sans ligne d'arrivée."
  }
};

export { S102 };
