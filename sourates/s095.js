// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S095 = {
  "meta": {
    "numero": 95,
    "nom_ar": "التين",
    "nom_translit": "At-Tin",
    "nom_fr": "Le Figuier",
    "nom_en": "The Fig",
    "nom_tr": "Tîn (İncir)",
    "type": "mecquoise",
    "versets_count": 8,
    "theme": "L'homme créé dans la meilleure stature — et le risque de la chute au plus bas",
    "note_pedagogique": "Quatre serments géographiques (les terroirs des grandes révélations) encadrent la thèse la plus haute du Coran sur l'homme : aḥsani taqwīm, la meilleure conformation. La dignité est native ; la déchéance, un choix.",
    "progression": {
      "couche": 2,
      "prerequis": [
        94
      ],
      "suivant": [
        96
      ]
    },
    "racines_cles": [
      "ت-ي-ن",
      "خ-ل-ق",
      "ق-و-م",
      "ر-د-د",
      "د-ي-ن"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "وَالتِّينِ وَالزَّيْتُونِ",
      "translit": "Wat-tīni waz-zaytūn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Par le figuier et l'olivier"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "By the fig and the olive"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İncir ve zeytine and olsun"
        }
      },
      "mots": [
        {
          "ar": "وَالتِّينِ",
          "translit": "wat-tīni",
          "fr": "figue",
          "en": "By the fig",
          "tr": "incire andolsun",
          "racine": "ت-ي-ن",
          "frequence_coran": 2
        },
        {
          "ar": "وَالزَّيْتُونِ",
          "translit": "waz-zaytūni",
          "fr": "olivier",
          "en": "and the olive",
          "tr": "ve zeytine",
          "racine": "ز-ي-ت",
          "frequence_coran": 7
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 2,
      "ar": "وَطُورِ سِينِينَ",
      "translit": "Waṭūri sīnīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Et par le Mont Sînîn"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And [by] Mount Sinai"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "And olsun Sina dağına"
        }
      },
      "mots": [
        {
          "ar": "وَطُورِ",
          "translit": "waṭūri",
          "fr": "mont (Sinaï)",
          "en": "And (the) Mount",
          "tr": "ve dağına",
          "racine": "ط-و-ر",
          "frequence_coran": 4
        },
        {
          "ar": "سِينِينَ",
          "translit": "sīnīna",
          "fr": "Sinaï",
          "en": "Sinai",
          "tr": "sina",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 3,
      "ar": "وَهَٰذَا الْبَلَدِ الْأَمِينِ",
      "translit": "Wahādhā l-baladi l-ʾamīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Et par cette Cité sûre"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And [by] this secure city [Makkah]"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "And olsun bu güvenli Mekke şehrine ki"
        }
      },
      "mots": [
        {
          "ar": "وَهَٰذَا",
          "translit": "wahādhā",
          "fr": "ceci",
          "en": "And this",
          "tr": "ve bu",
          "racine": "pronom démonstratif",
          "frequence_coran": 190
        },
        {
          "ar": "الْبَلَدِ",
          "translit": "l-baladi",
          "fr": "pays",
          "en": "[the] city",
          "tr": "Şehre (andolsun)",
          "racine": "ب-ل-د",
          "frequence_coran": 7
        },
        {
          "ar": "الْأَمِينِ",
          "translit": "l-ʾamīni",
          "fr": "sûre !",
          "en": "[the] secure",
          "tr": "güvenli",
          "racine": "ا-م-ن",
          "frequence_coran": 14,
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 4,
      "ar": "لَقَدْ خَلَقْنَا الْإِنسَٰنَ فِي أَحْسَنِ تَقْوِيمٍ",
      "translit": "Laqad khalaqnā l-ʾinsāna fī aḥsani taqwīm",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Nous avons certes créé l'homme dans la forme la plus parfaite"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "We have certainly created man in the best of stature"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Biz insanı en güzel şekilde yarattık"
        }
      },
      "mots": [
        {
          "ar": "لَقَدْ",
          "translit": "laqad",
          "fr": "certes",
          "en": "Indeed",
          "tr": "andolsun",
          "racine": "لَ + قَدْ",
          "frequence_coran": 52
        },
        {
          "ar": "خَلَقْنَا",
          "translit": "khalaqnā",
          "fr": "Nous avons créé",
          "en": "We created",
          "tr": "biz yarattık",
          "racine": "خ-ل-ق",
          "frequence_coran": 261,
          "src": "wbw"
        },
        {
          "ar": "الْإِنسَٰنَ",
          "translit": "l-ʾinsāna",
          "fr": "l’humain",
          "en": "man",
          "tr": "insanı",
          "racine": "ا-ن-س",
          "frequence_coran": 70,
          "src": "wbw"
        },
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "dans",
          "en": "in",
          "tr": "en güzel",
          "racine": "particule",
          "frequence_coran": 4381
        },
        {
          "ar": "أَحْسَنِ",
          "translit": "aḥsani",
          "fr": "bien faire",
          "en": "(the) best",
          "tr": "",
          "racine": "ح-س-ن",
          "frequence_coran": 33
        },
        {
          "ar": "تَقْوِيمٍ",
          "translit": "taqwīmin",
          "fr": "forme",
          "en": "mould",
          "tr": "biçimde",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 5,
      "ar": "ثُمَّ رَدَدْنَٰهُ أَسْفَلَ سَٰفِلِينَ",
      "translit": "Thumma radadnāhu asfala sāfilīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Ensuite, Nous l'avons ramené au niveau le plus bas"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Then We return him to the lowest of the low"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Sonra onu aşağıların en aşağısı kıldık"
        }
      },
      "mots": [
        {
          "ar": "ثُمَّ",
          "translit": "thumma",
          "fr": "puis",
          "en": "Then",
          "tr": "sonra",
          "racine": "particule de succession",
          "frequence_coran": 337
        },
        {
          "ar": "رَدَدْنَٰهُ",
          "translit": "radadnāhu",
          "fr": "Nous l’avons retourné",
          "en": "We returned him",
          "tr": "onu çevirdik",
          "racine": "ر-د-د",
          "frequence_coran": 60,
          "src": "wbw"
        },
        {
          "ar": "أَسْفَلَ",
          "translit": "asfala",
          "fr": "(au) plus bas",
          "en": "(to the) lowest",
          "tr": "aşağıların",
          "src": "wbw"
        },
        {
          "ar": "سَٰفِلِينَ",
          "translit": "sāfilīna",
          "fr": "des bas,",
          "en": "(of the) low",
          "tr": "aşağısına",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 6,
      "ar": "إِلَّا الَّذِينَ ءَامَنُواْ وَعَمِلُواْ الصَّـٰلِحَٰتِ فَلَهُمْ أَجْرٌ غَيْرُ مَمْنُونٍ",
      "translit": "Illā lladhīna āmanū waʿamilū ṣ-ṣāliḥāti falahum ajrun ghayru mamnūn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "sauf ceux qui croient et accomplissent les bonnes œuvres: ceux-là auront une récompense jamais interrompue"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Except for those who believe and do righteous deeds, for they will have a reward uninterrupted"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Yalnız, inanıp yararlı iş işleyenler bunun dışındadır. Onlara kesintisiz ecir vardır"
        }
      },
      "mots": [
        {
          "ar": "إِلَّا",
          "translit": "illā",
          "fr": "sauf",
          "en": "Except",
          "tr": "yalnız hariç",
          "racine": "particule d’exception",
          "frequence_coran": 662
        },
        {
          "ar": "الَّذِينَ",
          "translit": "lladhīna",
          "fr": "ceux qui",
          "en": "those who",
          "tr": "kimseler",
          "racine": "pronom relatif",
          "frequence_coran": 810
        },
        {
          "ar": "ءَامَنُواْ",
          "translit": "āmanū",
          "fr": "ont accepté la foi",
          "en": "believe",
          "tr": "inanan(lar)",
          "racine": "ا-م-ن",
          "frequence_coran": 440,
          "src": "wbw"
        },
        {
          "ar": "وَعَمِلُواْ",
          "translit": "waʿamilū",
          "fr": "et ils ont œuvré",
          "en": "and do",
          "tr": "ve yapanlar",
          "racine": "و + ع-م-ل",
          "frequence_coran": 53
        },
        {
          "ar": "الصَّـٰلِحَٰتِ",
          "translit": "ṣ-ṣāliḥāti",
          "fr": "les actions vertueuses,",
          "en": "righteous deeds",
          "tr": "iyi işler",
          "racine": "ص-ل-ح",
          "frequence_coran": 61,
          "src": "wbw"
        },
        {
          "ar": "فَلَهُمْ",
          "translit": "falahum",
          "fr": "à eux",
          "en": "then for them",
          "tr": "onlar için vardır",
          "racine": "لِ + هُمْ",
          "frequence_coran": 325
        },
        {
          "ar": "أَجْرٌ",
          "translit": "ajrun",
          "fr": "récompense",
          "en": "(is a) reward",
          "tr": "bir mükafat",
          "racine": "أ-ج-ر",
          "frequence_coran": 105
        },
        {
          "ar": "غَيْرُ",
          "translit": "ghayru",
          "fr": "autre que",
          "en": "never",
          "tr": "olmayan",
          "racine": "غ-ي-ر",
          "frequence_coran": 147
        },
        {
          "ar": "مَمْنُونٍ",
          "translit": "mamnūnin",
          "fr": "finissante.",
          "en": "ending",
          "tr": "kesintisi",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 7,
      "ar": "فَمَا يُكَذِّبُكَ بَعْدُ بِالدِّينِ",
      "translit": "Famā yukadhdhibuka baʿdu bid-dīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Après cela, qu'est-ce qui te fait traiter la rétribution de mensonge"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "So what yet causes you to deny the Recompense"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Öyleyken, sana dini yalan saydırtan nedir"
        }
      },
      "mots": [
        {
          "ar": "فَمَا",
          "translit": "famā",
          "fr": "alors quoi",
          "en": "Then what",
          "tr": "nedir?",
          "racine": "ف + مَا",
          "frequence_coran": 86
        },
        {
          "ar": "يُكَذِّبُكَ",
          "translit": "yukadhdhibuka",
          "fr": "te fait démentir",
          "en": "causes you to deny",
          "tr": "sana yalanlatan",
          "racine": "ك-ذ-ب",
          "frequence_coran": 80,
          "src": "wbw"
        },
        {
          "ar": "بَعْدُ",
          "translit": "baʿdu",
          "fr": "après",
          "en": "after (this)",
          "tr": "bundan sonra",
          "racine": "ب-ع-د",
          "frequence_coran": 82
        },
        {
          "ar": "بِالدِّينِ",
          "translit": "bid-dīni",
          "fr": "religion",
          "en": "the judgment",
          "tr": "dini",
          "racine": "د-ي-ن",
          "frequence_coran": 94
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 8,
      "ar": "أَلَيْسَ اللَّهُ بِأَحْكَمِ الْحَٰكِمِينَ",
      "translit": "Alaysa llāhu biʾaḥkami l-ḥākimīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Allah n'est-Il pas le plus sage des Juges"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Is not Allah the most just of judges"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Allah, hükmedenlerin en iyi hükmedeni değil midir"
        }
      },
      "mots": [
        {
          "ar": "أَلَيْسَ",
          "translit": "alaysa",
          "fr": "Est-ce que n’est pas",
          "en": "Is not",
          "tr": "değil midir?",
          "racine": "verbe de négation",
          "frequence_coran": 47,
          "src": "wbw"
        },
        {
          "ar": "اللَّهُ",
          "translit": "llāhu",
          "fr": "Allah",
          "en": "Allah",
          "tr": "Allah",
          "racine": "أ-ل-ه",
          "frequence_coran": 2699
        },
        {
          "ar": "بِأَحْكَمِ",
          "translit": "biʾaḥkami",
          "fr": "Celui qui juge le mieux",
          "en": "(the) Most Just",
          "tr": "en iyi hüküm vereni",
          "src": "wbw"
        },
        {
          "ar": "الْحَٰكِمِينَ",
          "translit": "l-ḥākimīna",
          "fr": "(parmi) les juges ?",
          "en": "(of) the Judges",
          "tr": "hüküm verenlerin",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "التِّين",
      "أَحْسَنِ تَقْوِيم",
      "أَسْفَلَ سَٰفِلِين",
      "الدِّين"
    ],
    "racines_a_retenir": [
      "ق-و-م",
      "س-ف-ل",
      "ح-ك-م"
    ],
    "declic_frequence": "taqwīm partage la racine ق-و-م (~660 occurrences) avec mustaqīm (droit), qāma (se tenir debout), qayyim (droit/précieux) : l'homme est créé 'dans la meilleure droiture' — et toute la vie consiste à rester debout sur cette stature d'origine."
  }
};

export { S095 };
