// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S099 = {
  "meta": {
    "numero": 99,
    "nom_ar": "الزلزلة",
    "nom_translit": "Az-Zalzalah",
    "nom_fr": "Le Séisme",
    "nom_en": "The Earthquake",
    "nom_tr": "Zilzâl (Deprem)",
    "type": "medinoise",
    "versets_count": 8,
    "theme": "La terre secouée raconte son histoire : chaque atome de bien et de mal sera vu",
    "note_pedagogique": "Huit versets qui font de la planète elle-même le premier témoin du Jugement. Les deux derniers versets sont, selon un hadith, 'le verset le plus englobant' jamais révélé : rien n'échappe à la pesée, pas même le poids d'un atome.",
    "progression": {
      "couche": 2,
      "prerequis": [
        98
      ],
      "suivant": [
        100
      ]
    },
    "racines_cles": [
      "ز-ل-ز-ل",
      "خ-ر-ج",
      "ح-د-ث",
      "ص-د-ر",
      "ذ-ر-ر"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "إِذَا زُلْزِلَتِ الْأَرْضُ زِلْزَالَهَا",
      "translit": "Idhā zulzilati l-ʾarḍu zilzālahā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Quand la terre tremblera d'un violent tremblement"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "When the earth is shaken with its [final] earthquake"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Yer dehşetle sarsıldıkça sarsıldığı, yeryüzü ağırlıklarını dışarıya çıkardığı ve insanın: \"Buna ne oluyor?\" dediği zaman"
        }
      },
      "mots": [
        {
          "ar": "إِذَا",
          "translit": "idhā",
          "fr": "quand",
          "en": "When",
          "tr": "zaman",
          "racine": "particule temporelle",
          "frequence_coran": 195
        },
        {
          "ar": "زُلْزِلَتِ",
          "translit": "zulzilati",
          "fr": "tremble",
          "en": "is shaken",
          "tr": "sarsıldığı",
          "src": "wbw"
        },
        {
          "ar": "الْأَرْضُ",
          "translit": "l-ʾarḍu",
          "fr": "la terre",
          "en": "the earth",
          "tr": "yer",
          "racine": "أ-ر-ض",
          "frequence_coran": 219
        },
        {
          "ar": "زِلْزَالَهَا",
          "translit": "zilzālahā",
          "fr": "(avec) son tremblement",
          "en": "(with) its earthquake",
          "tr": "yaman sarsıntı ile",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 3,
          "principal": true
        }
      }
    },
    {
      "numero": 2,
      "ar": "وَأَخْرَجَتِ الْأَرْضُ أَثْقَالَهَا",
      "translit": "Waʾakhrajati l-ʾarḍu athqālahā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et que la terre fera sortir ses fardeaux"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And the earth discharges its burdens"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Yer dehşetle sarsıldıkça sarsıldığı, yeryüzü ağırlıklarını dışarıya çıkardığı ve insanın: \"Buna ne oluyor?\" dediği zaman"
        }
      },
      "mots": [
        {
          "ar": "وَأَخْرَجَتِ",
          "translit": "waʾakhrajati",
          "fr": "et fait sortir",
          "en": "And brings forth",
          "tr": "ve çıkardığı (zaman)",
          "racine": "خ-ر-ج",
          "frequence_coran": 99,
          "src": "wbw"
        },
        {
          "ar": "الْأَرْضُ",
          "translit": "l-ʾarḍu",
          "fr": "la terre",
          "en": "the earth",
          "tr": "yer",
          "racine": "أ-ر-ض",
          "frequence_coran": 219
        },
        {
          "ar": "أَثْقَالَهَا",
          "translit": "athqālahā",
          "fr": "ses fardeaux lourds",
          "en": "its burdens",
          "tr": "ağırlıklarını",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 3,
          "principal": false
        }
      }
    },
    {
      "numero": 3,
      "ar": "وَقَالَ الْإِنسَٰنُ مَا لَهَا",
      "translit": "Waqāla l-ʾinsānu mā lahā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et que l'homme dira: «Qu'a-t-elle?»"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And man says, \"What is [wrong] with it"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Yer dehşetle sarsıldıkça sarsıldığı, yeryüzü ağırlıklarını dışarıya çıkardığı ve insanın: \"Buna ne oluyor?\" dediği zaman"
        }
      },
      "mots": [
        {
          "ar": "وَقَالَ",
          "translit": "waqāla",
          "fr": "et il a dit",
          "en": "And says",
          "tr": "ve dediği (zaman)",
          "racine": "و + ق-و-ل",
          "frequence_coran": 85
        },
        {
          "ar": "الْإِنسَٰنُ",
          "translit": "l-ʾinsānu",
          "fr": "l’humain :",
          "en": "man",
          "tr": "insan",
          "racine": "ا-ن-س",
          "frequence_coran": 70,
          "src": "wbw"
        },
        {
          "ar": "مَا",
          "translit": "mā",
          "fr": "ce que",
          "en": "What",
          "tr": "ne oluyor?",
          "racine": "particule",
          "frequence_coran": 1010
        },
        {
          "ar": "لَهَا",
          "translit": "lahā",
          "fr": "à elle",
          "en": "(is) with it",
          "tr": "ona",
          "racine": "لِ + هَا",
          "frequence_coran": 51
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 3,
          "principal": false
        }
      }
    },
    {
      "numero": 4,
      "ar": "يَوْمَئِذٍ تُحَدِّثُ أَخْبَارَهَا",
      "translit": "Yawmaʾidhin tuḥaddithu akhbārahā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "ce jour-là, elle contera son histoire"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "That Day, it will report its news"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İşte o gün, yer, Rabbinin ona vahyetmesiyle kendi haberlerini anlatır"
        }
      },
      "mots": [
        {
          "ar": "يَوْمَئِذٍ",
          "translit": "yawmaʾidhin",
          "fr": "ce jour-là",
          "en": "That Day",
          "tr": "işte o gün",
          "racine": "ي-و-م + إِذْ",
          "frequence_coran": 63
        },
        {
          "ar": "تُحَدِّثُ",
          "translit": "tuḥaddithu",
          "fr": "elle racontera",
          "en": "it will report",
          "tr": "söyler",
          "src": "wbw"
        },
        {
          "ar": "أَخْبَارَهَا",
          "translit": "akhbārahā",
          "fr": "ses informations",
          "en": "its news",
          "tr": "haberlerini",
          "src": "wbw"
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 4,
          "fin": 5,
          "principal": true
        }
      }
    },
    {
      "numero": 5,
      "ar": "بِأَنَّ رَبَّكَ أَوْحَىٰ لَهَا",
      "translit": "Biʾanna rabbaka awḥā lahā",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "selon ce que ton Seigneur lui aura révélé [ordonné]"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Because your Lord has commanded it"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "İşte o gün, yer, Rabbinin ona vahyetmesiyle kendi haberlerini anlatır"
        }
      },
      "mots": [
        {
          "ar": "بِأَنَّ",
          "translit": "biʾanna",
          "fr": "certes",
          "en": "Because",
          "tr": "çünkü",
          "racine": "particule d’insistance",
          "frequence_coran": 609
        },
        {
          "ar": "رَبَّكَ",
          "translit": "rabbaka",
          "fr": "ton Seigneur",
          "en": "your Lord",
          "tr": "Rabbin",
          "racine": "ر-ب-ب",
          "frequence_coran": 116
        },
        {
          "ar": "أَوْحَىٰ",
          "translit": "awḥā",
          "fr": "révéler",
          "en": "inspired",
          "tr": "vahyetmiştir",
          "racine": "و-ح-ي",
          "frequence_coran": 72
        },
        {
          "ar": "لَهَا",
          "translit": "lahā",
          "fr": "à elle",
          "en": "[to] it",
          "tr": "ona",
          "racine": "لِ + هَا",
          "frequence_coran": 51
        }
      ],
      "niveau_couche": 2,
      "groupes": {
        "tr": {
          "debut": 4,
          "fin": 5,
          "principal": false
        }
      }
    },
    {
      "numero": 6,
      "ar": "يَوْمَئِذٍ يَصْدُرُ النَّاسُ أَشْتَاتًا لِّيُرَوْاْ أَعْمَٰلَهُمْ",
      "translit": "Yawmaʾidhin yaṣduru n-nāsu ashtātan liyuraw aʿmālahum",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Ce jour-là, les gens sortiront séparément pour que leur soient montrées leurs œuvres"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "That Day, the people will depart separated [into categories] to be shown [the result of] their deeds"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "O gün insanlar işlerinin kendilerine gösterilmesi için bölük bölük dönerler"
        }
      },
      "mots": [
        {
          "ar": "يَوْمَئِذٍ",
          "translit": "yawmaʾidhin",
          "fr": "ce jour-là",
          "en": "That Day",
          "tr": "o gün",
          "racine": "ي-و-م + إِذْ",
          "frequence_coran": 63
        },
        {
          "ar": "يَصْدُرُ",
          "translit": "yaṣduru",
          "fr": "iront",
          "en": "will proceed",
          "tr": "çıkarlar",
          "src": "wbw"
        },
        {
          "ar": "النَّاسُ",
          "translit": "n-nāsu",
          "fr": "les gens",
          "en": "the mankind",
          "tr": "insanlar",
          "racine": "ن-و-س / أ-ن-س",
          "frequence_coran": 92
        },
        {
          "ar": "أَشْتَاتًا",
          "translit": "ashtātan",
          "fr": "(en étant) séparés",
          "en": "(in) scattered groups",
          "tr": "ayrı ayrı gruplar halinde",
          "src": "wbw"
        },
        {
          "ar": "لِّيُرَوْاْ",
          "translit": "liyuraw",
          "fr": "afin qu’ils soient montrés",
          "en": "to be shown",
          "tr": "gösterilsin diye",
          "racine": "ر-ا-ي",
          "frequence_coran": 40,
          "src": "wbw"
        },
        {
          "ar": "أَعْمَٰلَهُمْ",
          "translit": "aʿmālahum",
          "fr": "leurs actions.",
          "en": "their deeds",
          "tr": "yaptıkları işler",
          "racine": "ع-م-ل",
          "frequence_coran": 359,
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 7,
      "ar": "فَمَن يَعْمَلْ مِثْقَالَ ذَرَّةٍ خَيْرًا يَرَهُ",
      "translit": "Faman yaʿmal mithqāla dharratin khayran yarahū",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Quiconque fait un bien fût-ce du poids d'un atome, le verra"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "So whoever does an atom's weight of good will see it"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kim zerre kadar iyilik yapmışsa onu görür"
        }
      },
      "mots": [
        {
          "ar": "فَمَن",
          "translit": "faman",
          "fr": "donc celui qui",
          "en": "So whoever",
          "tr": "artık kim",
          "racine": "ف + مَنْ",
          "frequence_coran": 67
        },
        {
          "ar": "يَعْمَلْ",
          "translit": "yaʿmal",
          "fr": "fait",
          "en": "does",
          "tr": "yapmışsa",
          "racine": "ع-م-ل",
          "frequence_coran": 359,
          "src": "wbw"
        },
        {
          "ar": "مِثْقَالَ",
          "translit": "mithqāla",
          "fr": "poids (d'un atome)",
          "en": "(equal to the) weight",
          "tr": "ağırlığınca",
          "racine": "ث-ق-ل",
          "frequence_coran": 7
        },
        {
          "ar": "ذَرَّةٍ",
          "translit": "dharratin",
          "fr": "(d’)une chose la plus petite",
          "en": "(of) an atom",
          "tr": "zerre",
          "src": "wbw"
        },
        {
          "ar": "خَيْرًا",
          "translit": "khayran",
          "fr": "(en) bien,",
          "en": "good",
          "tr": "hayır",
          "racine": "خ-ي-ر",
          "frequence_coran": 78,
          "src": "wbw"
        },
        {
          "ar": "يَرَهُ",
          "translit": "yarahū",
          "fr": "le verra.",
          "en": "will see it",
          "tr": "onu görür",
          "racine": "ر-ا-ي",
          "frequence_coran": 265,
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    },
    {
      "numero": 8,
      "ar": "وَمَن يَعْمَلْ مِثْقَالَ ذَرَّةٍ شَرًّا يَرَهُ",
      "translit": "Waman yaʿmal mithqāla dharratin sharran yarahū",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "et quiconque fait un mal fût-ce du poids d'un atome, le verra"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And whoever does an atom's weight of evil will see it"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kim de zerre kadar kötülük yapmışsa onu görür"
        }
      },
      "mots": [
        {
          "ar": "وَمَن",
          "translit": "waman",
          "fr": "et celui qui",
          "en": "And whoever",
          "tr": "ve kim",
          "racine": "و + مَنْ",
          "frequence_coran": 234
        },
        {
          "ar": "يَعْمَلْ",
          "translit": "yaʿmal",
          "fr": "fait",
          "en": "does",
          "tr": "yapmışsa",
          "racine": "ع-م-ل",
          "frequence_coran": 359,
          "src": "wbw"
        },
        {
          "ar": "مِثْقَالَ",
          "translit": "mithqāla",
          "fr": "poids (d'un atome)",
          "en": "(equal to the) weight",
          "tr": "ağırlığınca",
          "racine": "ث-ق-ل",
          "frequence_coran": 7
        },
        {
          "ar": "ذَرَّةٍ",
          "translit": "dharratin",
          "fr": "(d’)une chose la plus petite",
          "en": "(of) an atom",
          "tr": "zerre",
          "src": "wbw"
        },
        {
          "ar": "شَرًّا",
          "translit": "sharran",
          "fr": "(en) mal,",
          "en": "evil",
          "tr": "şer",
          "racine": "ش-ر-ر",
          "frequence_coran": 8,
          "src": "wbw"
        },
        {
          "ar": "يَرَهُ",
          "translit": "yarahū",
          "fr": "le verra.",
          "en": "will see it",
          "tr": "onu görür",
          "racine": "ر-ا-ي",
          "frequence_coran": 265,
          "src": "wbw"
        }
      ],
      "niveau_couche": 2
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "زُلْزِلَت",
      "أَثْقَالَهَا",
      "تُحَدِّثُ أَخْبَارَهَا",
      "مِثْقَالَ ذَرَّة"
    ],
    "racines_a_retenir": [
      "ز-ل-ز-ل",
      "ث-ق-ل",
      "خ-ي-ر",
      "ش-ر-ر"
    ],
    "declic_frequence": "mithqāla dharra (le poids d'un atome) : dharra désigne la plus petite chose visible — grain de poussière dans un rayon de lumière, minuscule fourmi. L'unité de mesure du Jugement est l'infiniment petit : aucun geste n'est trop insignifiant pour compter."
  }
};

export { S099 };
