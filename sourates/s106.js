// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S106 = {
  "meta": {
    "numero": 106,
    "nom_ar": "قريش",
    "nom_translit": "Quraysh",
    "nom_fr": "Quraysh",
    "nom_en": "Quraysh",
    "nom_tr": "Kureyş",
    "type": "mecquoise",
    "versets_count": 4,
    "theme": "Les deux bienfaits fondateurs — nourriture et sécurité — appellent l'adoration du Seigneur de la Maison",
    "note_pedagogique": "Suite logique de la sourate de l'Éléphant : la Kaaba protégée (105) rend possibles les caravanes de Quraysh (106). Le raisonnement est simple : qui vous nourrit et vous protège mérite votre culte.",
    "progression": {
      "couche": 1,
      "prerequis": [
        105
      ],
      "suivant": [
        107
      ]
    },
    "racines_cles": [
      "أ-ل-ف",
      "ر-ح-ل",
      "ع-ب-د",
      "ط-ع-م",
      "أ-م-ن"
    ],
    "traduction_note": "Traductions pédagogiques originales (FR/EN/TR) — Comprendre le Coran"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "لِإِيلَٰفِ قُرَيْشٍ",
      "translit": "Liʾīlāfi quraysh",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "A cause du pacte des Coraïch"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "For the accustomed security of the Quraysh"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kureyş kabilesinin yaz ve kış yolculuklarında uzlaşması ve anlaşması sağlanmıştır"
        }
      },
      "mots": [
        {
          "ar": "لِإِيلَٰفِ",
          "translit": "liʾīlāfi",
          "fr": "pour le pacte / l'habitude (de)",
          "racine": "أ-ل-ف",
          "frequence_coran": 22,
          "famille": [
            "أَلِفَ (alifa) — s'habituer, s'attacher",
            "أُلْفَة (ulfa) — familiarité, concorde",
            "مُؤَلَّفَة (mu'allafa) — cœurs ralliés"
          ],
          "profondeur": "Īlāf désigne l'habitude protégée, le pacte tacite qui permettait aux caravanes qurayshites de traverser l'Arabie sans être pillées — grâce au prestige de la Kaaba. La sourate rappelle que cette prospérité commerciale n'est pas un dû : c'est un privilège lié à la Maison, donc à son Seigneur."
        },
        {
          "ar": "قُرَيْشٍ",
          "translit": "qurayshin",
          "fr": "Quraysh",
          "racine": "— (nom propre)",
          "frequence_coran": 1
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 2,
          "principal": true
        }
      }
    },
    {
      "numero": 2,
      "ar": "إِلَٰفِهِمْ رِحْلَةَ الشِّتَاءِ وَالصَّيْفِ",
      "translit": "Īlāfihim riḥlata sh-shitāʾi waṣ-ṣayf",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "De leur pacte [concernant] les voyages d'hiver et d'été"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Their accustomed security [in] the caravan of winter and summer"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kureyş kabilesinin yaz ve kış yolculuklarında uzlaşması ve anlaşması sağlanmıştır"
        }
      },
      "mots": [
        {
          "ar": "إِلَٰفِهِمْ",
          "translit": "īlāfihim",
          "fr": "leur pacte",
          "racine": "أ-ل-ف",
          "frequence_coran": 22
        },
        {
          "ar": "رِحْلَةَ",
          "translit": "riḥlata",
          "fr": "le voyage (de)",
          "racine": "ر-ح-ل",
          "frequence_coran": 6
        },
        {
          "ar": "الشِّتَاءِ",
          "translit": "sh-shitāʾi",
          "fr": "l'hiver",
          "racine": "ش-ت-و",
          "frequence_coran": 1
        },
        {
          "ar": "وَالصَّيْفِ",
          "translit": "waṣ-ṣayfi",
          "fr": "et l'été",
          "racine": "ص-ي-ف",
          "frequence_coran": 1
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 2,
          "principal": false
        }
      }
    },
    {
      "numero": 3,
      "ar": "فَلْيَعْبُدُواْ رَبَّ هَٰذَا الْبَيْتِ",
      "translit": "Falyaʿbudū rabba hādhā l-bayt",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Qu'ils adorent donc le Seigneur de cette Maison [la Ka'ba]"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Let them worship the Lord of this House"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Öyleyse kendilerini açken doyuran ve korku içindeyken güven veren bu Ev'in (Kabe'nin) Rabbine kulluk etsinler"
        }
      },
      "mots": [
        {
          "ar": "فَلْيَعْبُدُواْ",
          "translit": "falyaʿbudū",
          "fr": "qu'ils adorent donc",
          "racine": "ع-ب-د",
          "frequence_coran": 275
        },
        {
          "ar": "رَبَّ",
          "translit": "rabba",
          "fr": "le Seigneur (de)",
          "racine": "ر-ب-ب",
          "frequence_coran": 970
        },
        {
          "ar": "هَٰذَا",
          "translit": "hādhā",
          "fr": "cette",
          "racine": "— (démonstratif)",
          "frequence_coran": 0
        },
        {
          "ar": "الْبَيْتِ",
          "translit": "l-bayti",
          "fr": "Maison",
          "racine": "ب-ي-ت",
          "frequence_coran": 71
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 3,
          "fin": 4,
          "principal": true
        }
      }
    },
    {
      "numero": 4,
      "ar": "الَّذِي أَطْعَمَهُم مِّن جُوعٍ وَءَامَنَهُم مِّنْ خَوْفٍ",
      "translit": "Alladhī aṭʿamahum min jūʿin waʾāmanahum min khawf",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "qui les a nourris contre la faim et rassurés de la crainte"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Who has fed them, [saving them] from hunger and made them safe, [saving them] from fear"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Öyleyse kendilerini açken doyuran ve korku içindeyken güven veren bu Ev'in (Kabe'nin) Rabbine kulluk etsinler"
        }
      },
      "mots": [
        {
          "ar": "الَّذِي",
          "translit": "alladhī",
          "fr": "celui qui",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "أَطْعَمَهُم",
          "translit": "aṭʿamahum",
          "fr": "les a nourris",
          "racine": "ط-ع-م",
          "frequence_coran": 48
        },
        {
          "ar": "مِّن",
          "translit": "min",
          "fr": "contre",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "جُوعٍ",
          "translit": "jūʿin",
          "fr": "la faim",
          "racine": "ج-و-ع",
          "frequence_coran": 5
        },
        {
          "ar": "وَءَامَنَهُم",
          "translit": "waʾāmanahum",
          "fr": "et les a mis en sécurité",
          "racine": "أ-م-ن",
          "frequence_coran": 879
        },
        {
          "ar": "مِّنْ",
          "translit": "min",
          "fr": "contre",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "خَوْفٍ",
          "translit": "khawfi",
          "fr": "la peur",
          "racine": "خ-و-ف",
          "frequence_coran": 124
        }
      ],
      "analyse_globale": "Faim et peur : les deux insécurités primordiales de toute société humaine. Le verset les nomme dans l'ordre exact des besoins (subsistance puis sécurité) et attribue leur résolution à Dieu seul — fondement d'une gratitude qui devient culte.",
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 3,
          "fin": 4,
          "principal": false
        }
      }
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "إِيلَٰف",
      "رِحْلَة",
      "الْبَيْت",
      "جُوع",
      "خَوْف"
    ],
    "racines_a_retenir": [
      "أ-ل-ف",
      "ع-ب-د",
      "أ-م-ن"
    ],
    "declic_frequence": "amana (racine أ-م-ن, ~879 occurrences avec ses dérivés) relie sécurité et foi : le même verbe donne āmana (croire) et āmanahum (les mettre en sécurité). Croire, c'est littéralement entrer en sécurité."
  }
};

export { S106 };
