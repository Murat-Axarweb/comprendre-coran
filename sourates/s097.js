// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S097 = {
  "meta": {
    "numero": 97,
    "nom_ar": "القدر",
    "nom_translit": "Al-Qadr",
    "nom_fr": "La Destinée",
    "nom_en": "The Decree",
    "nom_tr": "Kadir",
    "type": "mecquoise",
    "versets_count": 5,
    "theme": "La Nuit du Destin : la descente du Coran, une nuit qui vaut plus de mille mois",
    "note_pedagogique": "Cinq versets sur la nuit où tout a commencé. Le mot qadr y résonne trois fois : valeur, mesure, destin. Sourate à relier au mois de Ramadan (2:185) durant lequel cette nuit se cherche.",
    "progression": {
      "couche": 1,
      "prerequis": [
        111
      ],
      "suivant": [
        103
      ]
    },
    "racines_cles": [
      "ن-ز-ل",
      "ق-د-ر",
      "م-ل-ك",
      "س-ل-م"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "إِنَّا أَنزَلْنَٰهُ فِي لَيْلَةِ الْقَدْرِ",
      "translit": "Innā anzalnāhu fī laylati l-qadr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Nous l'avons certes, fait descendre (le Coran) pendant la nuit d'Al-Qadr"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Indeed, We sent the Qur'an down during the Night of Decree"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Doğrusu, Biz, Kuran'ı kadir gecesinde indirmişizdir"
        }
      },
      "mots": [
        {
          "ar": "إِنَّا",
          "translit": "innā",
          "fr": "certes Nous",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "أَنزَلْنَٰهُ",
          "translit": "anzalnāhu",
          "fr": "Nous l'avons fait descendre",
          "racine": "ن-ز-ل",
          "frequence_coran": 293
        },
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "dans / durant",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "لَيْلَةِ",
          "translit": "laylati",
          "fr": "la Nuit (de)",
          "racine": "ل-ي-ل",
          "frequence_coran": 92
        },
        {
          "ar": "الْقَدْرِ",
          "translit": "l-qadri",
          "fr": "la Destinée",
          "racine": "ق-د-ر",
          "frequence_coran": 132,
          "famille": [
            "قَدَرَ (qadara) — mesurer, pouvoir",
            "قَدَر (qadar) — destin, mesure",
            "قَدِير (qadīr) — Tout-Puissant",
            "مِقْدَار (miqdār) — quantité"
          ],
          "profondeur": "Qadr réunit trois sens que le français sépare : la valeur (cette nuit est précieuse), la mesure (les décrets de l'année y sont fixés selon la tradition) et la puissance (le destin s'y déploie). Une nuit où la valeur, la mesure et le pouvoir divins coïncident."
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 2,
      "ar": "وَمَا أَدْرَىٰكَ مَا لَيْلَةُ الْقَدْرِ",
      "translit": "Wamā adrāka mā laylatu l-qadr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Et qui te dira ce qu'est la nuit d'Al-Qadr"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And what can make you know what is the Night of Decree"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kadir gecesinin ne olduğunu sen bilir misin"
        }
      },
      "mots": [
        {
          "ar": "وَمَا",
          "translit": "wamā",
          "fr": "et qu'est-ce qui",
          "racine": "— (interrogatif)",
          "frequence_coran": 0
        },
        {
          "ar": "أَدْرَىٰكَ",
          "translit": "adrāka",
          "fr": "t'a fait savoir",
          "racine": "د-ر-ي",
          "frequence_coran": 29
        },
        {
          "ar": "مَا",
          "translit": "mā",
          "fr": "ce qu'est",
          "racine": "— (interrogatif)",
          "frequence_coran": 0
        },
        {
          "ar": "لَيْلَةُ",
          "translit": "laylatu",
          "fr": "la Nuit (de)",
          "racine": "ل-ي-ل",
          "frequence_coran": 92
        },
        {
          "ar": "الْقَدْرِ",
          "translit": "l-qadri",
          "fr": "la Destinée",
          "racine": "ق-د-ر",
          "frequence_coran": 132
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 3,
      "ar": "لَيْلَةُ الْقَدْرِ خَيْرٌ مِّنْ أَلْفِ شَهْرٍ",
      "translit": "Laylatu l-qadri khayrun min alfi shahr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "La nuit d'Al-Qadr est meilleure que mille mois"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "The Night of Decree is better than a thousand months"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Kadir gecesi bin aydan hayırlıdır"
        }
      },
      "mots": [
        {
          "ar": "لَيْلَةُ",
          "translit": "laylatu",
          "fr": "la Nuit (de)",
          "racine": "ل-ي-ل",
          "frequence_coran": 92
        },
        {
          "ar": "الْقَدْرِ",
          "translit": "l-qadri",
          "fr": "la Destinée",
          "racine": "ق-د-ر",
          "frequence_coran": 132
        },
        {
          "ar": "خَيْرٌ",
          "translit": "khayrun",
          "fr": "est meilleure",
          "racine": "خ-ي-ر",
          "frequence_coran": 186
        },
        {
          "ar": "مِّنْ",
          "translit": "min",
          "fr": "que",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "أَلْفِ",
          "translit": "alfi",
          "fr": "mille",
          "racine": "أ-ل-ف",
          "frequence_coran": 8
        },
        {
          "ar": "شَهْرٍ",
          "translit": "shahrin",
          "fr": "mois",
          "racine": "ش-ه-ر",
          "frequence_coran": 21
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 4,
      "ar": "تَنَزَّلُ الْمَلَـٰئِكَةُ وَالرُّوحُ فِيهَا بِإِذْنِ رَبِّهِم مِّن كُلِّ أَمْرٍ",
      "translit": "Tanazzalu l-malāʾikatu war-rūḥu fīhā biʾidhni rabbihim min kulli amr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Durant celle-ci descendent les Anges ainsi que l'Esprit, par permission de leur Seigneur pour tout ordre"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "The angels and the Spirit descend therein by permission of their Lord for every matter"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Melekler ve Cebrail o gecede Rablerinin izniyle her türlü iş için inerler"
        }
      },
      "mots": [
        {
          "ar": "تَنَزَّلُ",
          "translit": "tanazzalu",
          "fr": "descendent",
          "racine": "ن-ز-ل",
          "frequence_coran": 293
        },
        {
          "ar": "الْمَلَـٰئِكَةُ",
          "translit": "l-malāʾikatu",
          "fr": "les anges",
          "racine": "م-ل-ك",
          "frequence_coran": 88
        },
        {
          "ar": "وَالرُّوحُ",
          "translit": "war-rūḥu",
          "fr": "et l'Esprit",
          "racine": "ر-و-ح",
          "frequence_coran": 21
        },
        {
          "ar": "فِيهَا",
          "translit": "fīhā",
          "fr": "en elle",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "بِإِذْنِ",
          "translit": "biʾidhni",
          "fr": "avec la permission (de)",
          "racine": "أ-ذ-ن",
          "frequence_coran": 39
        },
        {
          "ar": "رَبِّهِم",
          "translit": "rabbihim",
          "fr": "leur Seigneur",
          "racine": "ر-ب-ب",
          "frequence_coran": 970
        },
        {
          "ar": "مِّن",
          "translit": "min",
          "fr": "pour",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "كُلِّ",
          "translit": "kulli",
          "fr": "tout",
          "racine": "ك-ل-ل",
          "frequence_coran": 358
        },
        {
          "ar": "أَمْرٍ",
          "translit": "amrin",
          "fr": "ordre / décret",
          "racine": "أ-م-ر",
          "frequence_coran": 166
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 5,
      "ar": "سَلَٰمٌ هِيَ حَتَّىٰ مَطْلَعِ الْفَجْرِ",
      "translit": "Salāmun hiya ḥattā maṭlaʿi l-fajr",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Elle est paix et salut jusqu'à l'apparition de l'aube"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Peace it is until the emergence of dawn"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "O gece, tan yerinin ağarmasına kadar bir esenliktir"
        }
      },
      "mots": [
        {
          "ar": "سَلَٰمٌ",
          "translit": "salāmun",
          "fr": "paix",
          "racine": "س-ل-م",
          "frequence_coran": 140
        },
        {
          "ar": "هِيَ",
          "translit": "hiya",
          "fr": "elle est",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "حَتَّىٰ",
          "translit": "ḥattā",
          "fr": "jusqu'à",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "مَطْلَعِ",
          "translit": "maṭlaʿi",
          "fr": "le lever (de)",
          "racine": "ط-ل-ع",
          "frequence_coran": 19
        },
        {
          "ar": "الْفَجْرِ",
          "translit": "l-fajri",
          "fr": "l'aube",
          "racine": "ف-ج-ر",
          "frequence_coran": 24
        }
      ],
      "niveau_couche": 1
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "أَنزَلْنَٰهُ",
      "لَيْلَة الْقَدْر",
      "الرُّوح",
      "سَلَٰم",
      "الْفَجْر"
    ],
    "racines_a_retenir": [
      "ن-ز-ل",
      "ق-د-ر",
      "ر-و-ح"
    ],
    "declic_frequence": "nazala (descendre, racine ن-ز-ل ~293 occurrences) est LE verbe de la révélation : le Coran ne 'monte' pas de l'inspiration humaine, il 'descend' — anzalnāhu, 'Nous l'avons fait descendre', premier mot d'action de la sourate."
  }
};

export { S097 };
