// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S104 = {
  "meta": {
    "numero": 104,
    "nom_ar": "الهمزة",
    "nom_translit": "Al-Humazah",
    "nom_fr": "Le Calomniateur",
    "nom_en": "The Slanderer",
    "nom_tr": "Hümeze (Arkadan Çekiştiren)",
    "type": "mecquoise",
    "versets_count": 9,
    "theme": "Portrait du moqueur thésauriseur et du feu qui atteint les cœurs",
    "note_pedagogique": "Une sourate entièrement construite sur la correspondance faute-châtiment : celui qui brise les autres (humaza) sera jeté dans ce qui brise (al-hutama).",
    "progression": {
      "couche": 1,
      "prerequis": [
        103
      ],
      "suivant": [
        107
      ]
    },
    "racines_cles": [
      "ه-م-ز",
      "ج-م-ع",
      "ح-ط-م",
      "و-ق-د"
    ],
    "traduction_note": "Hamidullah (fr) · Saheeh International (en) · Diyanet İşleri Başkanlığı (tr) — via Tanzil.net"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "وَيْلٌ لِّكُلِّ هُمَزَةٍ لُّمَزَةٍ",
      "translit": "Waylun likulli humazatin lumazat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Malheur à tout calomniateur diffamateur"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Woe to every scorner and mocker"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Mal toplayarak onu tekrar tekrar sayan, diliyle çekiştirip alay eden kimsenin vay haline"
        }
      },
      "mots": [
        {
          "ar": "وَيْلٌ",
          "translit": "waylun",
          "fr": "malheur à",
          "en": "Woe",
          "tr": "vay haline",
          "racine": "و-ي-ل",
          "frequence_coran": 40
        },
        {
          "ar": "لِّكُلِّ",
          "translit": "likulli",
          "fr": "tout / chaque",
          "en": "to every",
          "tr": "herkesin",
          "racine": "ك-ل-ل",
          "frequence_coran": 358
        },
        {
          "ar": "هُمَزَةٍ",
          "translit": "humazatin",
          "fr": "calomniateur",
          "en": "slanderer",
          "tr": "diliyle çekiştiren",
          "racine": "ه-م-ز",
          "frequence_coran": 3
        },
        {
          "ar": "لُّمَزَةٍ",
          "translit": "lumazatin",
          "fr": "dénigreur",
          "en": "backbiter",
          "tr": "kaş göz işaretleriyle alay eden",
          "racine": "ل-م-ز",
          "frequence_coran": 4
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 2,
          "principal": true
        }
      }
    },
    {
      "numero": 2,
      "ar": "الَّذِي جَمَعَ مَالًا وَعَدَّدَهُ",
      "translit": "Alladhī jamaʿa mālan waʿaddadahū",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "qui amasse une fortune et la compte"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Who collects wealth and [continuously] counts it"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Mal toplayarak onu tekrar tekrar sayan, diliyle çekiştirip alay eden kimsenin vay haline"
        }
      },
      "mots": [
        {
          "ar": "الَّذِي",
          "translit": "alladhī",
          "fr": "celui qui",
          "en": "The one who",
          "tr": "o ki",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "جَمَعَ",
          "translit": "jamaʿa",
          "fr": "a amassé",
          "en": "collects",
          "tr": "yığdı",
          "racine": "ج-م-ع",
          "frequence_coran": 129
        },
        {
          "ar": "مَالًا",
          "translit": "mālan",
          "fr": "une fortune",
          "en": "wealth",
          "tr": "mal",
          "racine": "م-و-ل",
          "frequence_coran": 86
        },
        {
          "ar": "وَعَدَّدَهُ",
          "translit": "waʿaddadahū",
          "fr": "et l'a comptée sans cesse",
          "en": "and counts it",
          "tr": "ve onu saydı durdu",
          "racine": "ع-د-د",
          "frequence_coran": 57
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 1,
          "fin": 2,
          "principal": false
        }
      }
    },
    {
      "numero": 3,
      "ar": "يَحْسَبُ أَنَّ مَالَهُ أَخْلَدَهُ",
      "translit": "Yaḥsabu anna mālahū akhladahū",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "pensant que sa fortune l'immortalisera"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "He thinks that his wealth will make him immortal"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Malının kendisini ölümsüz kılacağını sanır"
        }
      },
      "mots": [
        {
          "ar": "يَحْسَبُ",
          "translit": "yaḥsabu",
          "fr": "il pense",
          "en": "Thinking",
          "tr": "sanıyor",
          "racine": "ح-س-ب",
          "frequence_coran": 109
        },
        {
          "ar": "أَنَّ",
          "translit": "anna",
          "fr": "que",
          "en": "that",
          "tr": "muhakkak",
          "racine": "— (particule)",
          "frequence_coran": 0
        },
        {
          "ar": "مَالَهُ",
          "translit": "mālahū",
          "fr": "sa fortune",
          "en": "his wealth",
          "tr": "malının",
          "racine": "م-و-ل",
          "frequence_coran": 86
        },
        {
          "ar": "أَخْلَدَهُ",
          "translit": "akhladahū",
          "fr": "l'a rendu immortel",
          "en": "will make him immortal",
          "tr": "onu ebedi yaşatacağını",
          "racine": "خ-ل-د",
          "frequence_coran": 87
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 4,
      "ar": "كَلَّا لَيُنبَذَنَّ فِي الْحُطَمَةِ",
      "translit": "Kallā layunbadhanna fī l-ḥuṭamat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Mais non! Il sera certes, jeté dans la Hutamah"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "No! He will surely be thrown into the Crusher"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Hayır; o, and olsun ki, Hutame'ye atılacaktır"
        }
      },
      "mots": [
        {
          "ar": "كَلَّا",
          "translit": "kallā",
          "fr": "que non !",
          "en": "Nay",
          "tr": "hayır",
          "racine": "— (réfutation)",
          "frequence_coran": 33
        },
        {
          "ar": "لَيُنبَذَنَّ",
          "translit": "layunbadhanna",
          "fr": "il sera certes précipité",
          "en": "Surely he will be thrown",
          "tr": "andolsun o atılacaktır",
          "racine": "ن-ب-ذ",
          "frequence_coran": 12
        },
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "dans",
          "en": "in",
          "tr": "Hutame'ye",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "الْحُطَمَةِ",
          "translit": "l-ḥuṭamati",
          "fr": "la Hutama (la Broyeuse)",
          "en": "the Crusher",
          "tr": "",
          "racine": "ح-ط-م",
          "frequence_coran": 3
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 5,
      "ar": "وَمَا أَدْرَىٰكَ مَا الْحُطَمَةُ",
      "translit": "Wamā adrāka mā l-ḥuṭamat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Et qui te dira ce qu'est la Hutamah"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "And what can make you know what is the Crusher"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Hutame'nin ne olduğunu sen bilir misin"
        }
      },
      "mots": [
        {
          "ar": "وَمَا",
          "translit": "wamā",
          "fr": "et qu'est-ce qui",
          "en": "And what",
          "tr": "nedir?",
          "racine": "— (interrogatif)",
          "frequence_coran": 0
        },
        {
          "ar": "أَدْرَىٰكَ",
          "translit": "adrāka",
          "fr": "t'a fait savoir",
          "en": "will make you know",
          "tr": "bilir misin?",
          "racine": "د-ر-ي",
          "frequence_coran": 29
        },
        {
          "ar": "مَا",
          "translit": "mā",
          "fr": "ce qu'est",
          "en": "what",
          "tr": "nedir",
          "racine": "— (interrogatif)",
          "frequence_coran": 0
        },
        {
          "ar": "الْحُطَمَةُ",
          "translit": "l-ḥuṭamatu",
          "fr": "la Hutama",
          "en": "the Crusher (is)",
          "tr": "Hutame",
          "racine": "ح-ط-م",
          "frequence_coran": 3
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 6,
      "ar": "نَارُ اللَّهِ الْمُوقَدَةُ",
      "translit": "Nāru llāhi l-mūqadat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Le Feu attisé d'Allah"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "It is the fire of Allah, [eternally] fueled"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "O, yüreklere çökecek olan, Allah'ın tutuşturulmuş ateşidir"
        }
      },
      "mots": [
        {
          "ar": "نَارُ",
          "translit": "nāru",
          "fr": "le feu (de)",
          "en": "A Fire",
          "tr": "ateşidir",
          "racine": "ن-و-ر",
          "frequence_coran": 145
        },
        {
          "ar": "اللَّهِ",
          "translit": "llāhi",
          "fr": "Allah",
          "en": "Allah",
          "tr": "Allah'ın",
          "racine": "أ-ل-ه",
          "frequence_coran": 2699
        },
        {
          "ar": "الْمُوقَدَةُ",
          "translit": "l-mūqadatu",
          "fr": "attisé",
          "en": "kindled",
          "tr": "tutuşturulmuş",
          "racine": "و-ق-د",
          "frequence_coran": 11
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 6,
          "fin": 7,
          "principal": true
        }
      }
    },
    {
      "numero": 7,
      "ar": "الَّتِي تَطَّلِعُ عَلَى الْأَفْـءِدَةِ",
      "translit": "Allatī taṭṭaliʿu ʿalā l-ʾafʾidat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "qui monte jusqu'aux cœurs"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Which mounts directed at the hearts"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "O, yüreklere çökecek olan, Allah'ın tutuşturulmuş ateşidir"
        }
      },
      "mots": [
        {
          "ar": "الَّتِي",
          "translit": "allatī",
          "fr": "celui qui",
          "en": "Which",
          "tr": "ki",
          "racine": "— (relatif)",
          "frequence_coran": 0
        },
        {
          "ar": "تَطَّلِعُ",
          "translit": "taṭṭaliʿu",
          "fr": "monte / s'élève",
          "en": "mounts up",
          "tr": "işler",
          "racine": "ط-ل-ع",
          "frequence_coran": 19
        },
        {
          "ar": "عَلَى",
          "translit": "ʿalā",
          "fr": "jusqu'aux",
          "en": "to",
          "tr": "gönüllere",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "الْأَفْـءِدَةِ",
          "translit": "l-ʾafʾidati",
          "fr": "cœurs profonds",
          "en": "the hearts",
          "tr": "",
          "racine": "ف-أ-د",
          "frequence_coran": 16,
          "famille": [
            "فُؤَاد (fu'ād) — cœur ardent",
            "أَفْئِدَة (af'ida) — cœurs"
          ],
          "profondeur": "Al-af'ida désigne le cœur en tant que siège brûlant des émotions et des choix (la racine évoque le feu, fa'd = griller). Le feu de la Hutama atteint précisément l'organe qui a choisi l'avarice et la moquerie : le châtiment remonte à la source de la faute."
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 6,
          "fin": 7,
          "principal": false
        }
      }
    },
    {
      "numero": 8,
      "ar": "إِنَّهَا عَلَيْهِم مُّؤْصَدَةٌ",
      "translit": "Innahā ʿalayhim muʾṣadat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Il se refermera sur eux"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Indeed, Hellfire will be closed down upon them"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onlar, uzun sütunlar arasında, her yönden o ateşle kapatılmışlardır"
        }
      },
      "mots": [
        {
          "ar": "إِنَّهَا",
          "translit": "innahā",
          "fr": "certes lui (le feu)",
          "en": "Indeed,it",
          "tr": "şüphesiz o",
          "racine": "— (particule)",
          "frequence_coran": 0
        },
        {
          "ar": "عَلَيْهِم",
          "translit": "ʿalayhim",
          "fr": "sur eux",
          "en": "(will be) upon them",
          "tr": "onların üzerine",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "مُّؤْصَدَةٌ",
          "translit": "muʾṣadatun",
          "fr": "refermé",
          "en": "closed over",
          "tr": "kapatılıp kilitlenecektir",
          "racine": "و-ص-د",
          "frequence_coran": 2
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 8,
          "fin": 9,
          "principal": true
        }
      }
    },
    {
      "numero": 9,
      "ar": "فِي عَمَدٍ مُّمَدَّدَةٍ",
      "translit": "Fī ʿamadin mumaddadat",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "en colonnes (de flammes) étendues"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "In extended columns"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Onlar, uzun sütunlar arasında, her yönden o ateşle kapatılmışlardır"
        }
      },
      "mots": [
        {
          "ar": "فِي",
          "translit": "fī",
          "fr": "en",
          "en": "In",
          "tr": "arasında",
          "racine": "— (préposition)",
          "frequence_coran": 0
        },
        {
          "ar": "عَمَدٍ",
          "translit": "ʿamadin",
          "fr": "colonnes",
          "en": "columns",
          "tr": "direkler",
          "racine": "ع-م-د",
          "frequence_coran": 7
        },
        {
          "ar": "مُّمَدَّدَةٍ",
          "translit": "mumaddadati",
          "fr": "étendues",
          "en": "extended",
          "tr": "uzatılmış",
          "racine": "م-د-د",
          "frequence_coran": 32
        }
      ],
      "niveau_couche": 1,
      "groupes": {
        "tr": {
          "debut": 8,
          "fin": 9,
          "principal": false
        }
      }
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "هُمَزَة",
      "جَمَعَ",
      "الْحُطَمَة",
      "الْأَفْئِدَة"
    ],
    "racines_a_retenir": [
      "ه-م-ز",
      "ح-ط-م",
      "خ-ل-د"
    ],
    "declic_frequence": "jamaʿa mālan (amasser des biens, racine ج-م-ع ~130 occurrences) : le verbe neutre 'rassembler' devient accusation quand l'accumulation remplace la relation — compter sa fortune au lieu de compter avec Dieu."
  }
};

export { S104 };
