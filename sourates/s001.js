// Traductions : Hamidullah (fr), Saheeh International (en), Diyanet (tr)
// Source : quran-json / Tanzil.net — reproduction verbatim, usage non commercial.
const S001 = {
  "meta": {
    "numero": 1,
    "nom_ar": "الفاتحة",
    "nom_translit": "Al-Fatihah",
    "nom_fr": "L'Ouverture",
    "nom_en": "The Opening",
    "nom_tr": "Fâtiha (Açılış)",
    "type": "mecquoise",
    "versets_count": 7,
    "theme": "La prière-mère : louange, adoration et demande de guidance en 7 versets",
    "note_pedagogique": "La sourate la plus récitée au monde : au moins 17 fois par jour dans les prières. Elle contient tout le programme du Coran — c'est pourquoi elle s'appelle aussi Umm al-Kitab, la Mère du Livre.",
    "progression": {
      "couche": 1,
      "prerequis": [],
      "suivant": [
        112,
        113,
        114
      ]
    },
    "racines_cles": [
      "ح-م-د",
      "ر-ح-م",
      "ع-ب-د",
      "ه-د-ي",
      "ص-ر-ط"
    ],
    "traduction_note": "Traductions pédagogiques originales (FR/EN/TR) — Comprendre le Coran"
  },
  "versets": [
    {
      "numero": 1,
      "ar": "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ",
      "translit": "Bismi llāhi r-raḥmāni r-raḥīm",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Au nom d'Allah, le Tout Miséricordieux, le Très Miséricordieux"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "In the name of Allah, the Entirely Merciful, the Especially Merciful"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Rahman ve Rahim olan Allah'ın adıyla"
        }
      },
      "mots": [
        {
          "ar": "بِسْمِ",
          "translit": "bismi",
          "fr": "au nom de",
          "racine": "س-م-و",
          "frequence_coran": 22
        },
        {
          "ar": "اللَّهِ",
          "translit": "llāhi",
          "fr": "Allah",
          "racine": "أ-ل-ه",
          "frequence_coran": 2699
        },
        {
          "ar": "الرَّحْمَٰنِ",
          "translit": "r-raḥmāni",
          "fr": "le Tout-Miséricordieux",
          "racine": "ر-ح-م",
          "frequence_coran": 57
        },
        {
          "ar": "الرَّحِيمِ",
          "translit": "r-raḥīmi",
          "fr": "le Très-Miséricordieux",
          "racine": "ر-ح-م",
          "frequence_coran": 115
        }
      ],
      "analyse_globale": "La basmala ouvre 113 des 114 sourates. Les deux noms raḥmān et raḥīm dérivent de la même racine ر-ح-م, liée au mot raḥim (matrice, utérus) : la miséricorde divine enveloppe comme le ventre maternel enveloppe l'enfant. Raḥmān désigne la miséricorde immense qui embrasse toute la création ; raḥīm, la miséricorde constante et renouvelée envers les croyants.",
      "niveau_couche": 1
    },
    {
      "numero": 2,
      "ar": "الْحَمْدُ لِلَّهِ رَبِّ الْعَٰلَمِينَ",
      "translit": "Al-ḥamdu lillāhi rabbi l-ʿālamīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Louange à Allah, Seigneur de l'univers"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "[All] praise is [due] to Allah, Lord of the worlds"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Hamd, Alemlerin Rabbi Allah'a mahsustur"
        }
      },
      "mots": [
        {
          "ar": "الْحَمْدُ",
          "translit": "al-ḥamdu",
          "fr": "la louange",
          "racine": "ح-م-د",
          "frequence_coran": 43
        },
        {
          "ar": "لِلَّهِ",
          "translit": "lillāhi",
          "fr": "à Allah / pour Allah",
          "racine": "أ-ل-ه",
          "frequence_coran": 2699
        },
        {
          "ar": "رَبِّ",
          "translit": "rabbi",
          "fr": "Seigneur",
          "racine": "ر-ب-ب",
          "frequence_coran": 970,
          "famille": [
            "رَبَّىٰ (rabbā) — élever, éduquer",
            "تَرْبِيَة (tarbiya) — éducation",
            "رَبَّانِيّ (rabbāniyy) — homme de Dieu accompli"
          ],
          "profondeur": "Rabb (~970 occurrences, 2e mot le plus fréquent après Allah) ne signifie pas seulement 'Seigneur' : la racine évoque celui qui élève, éduque et fait grandir progressivement. Dire 'rabbi l-ʿālamīn', c'est reconnaître que Dieu accompagne chaque créature dans sa croissance, étape par étape — comme un éducateur parfait."
        },
        {
          "ar": "الْعَٰلَمِينَ",
          "translit": "l-ʿālamīna",
          "fr": "des mondes",
          "racine": "ع-ل-م",
          "frequence_coran": 73
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 3,
      "ar": "الرَّحْمَٰنِ الرَّحِيمِ",
      "translit": "Ar-raḥmāni r-raḥīm",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Le Tout Miséricordieux, le Très Miséricordieux"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "The Entirely Merciful, the Especially Merciful"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "O Rahman ve Rahim'dir"
        }
      },
      "mots": [
        {
          "ar": "الرَّحْمَٰنِ",
          "translit": "ar-raḥmāni",
          "fr": "le Tout-Miséricordieux",
          "racine": "ر-ح-م",
          "frequence_coran": 57
        },
        {
          "ar": "الرَّحِيمِ",
          "translit": "r-raḥīmi",
          "fr": "le Très-Miséricordieux",
          "racine": "ر-ح-م",
          "frequence_coran": 115
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 4,
      "ar": "مَٰلِكِ يَوْمِ الدِّينِ",
      "translit": "Māliki yawmi d-dīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Maître du Jour de la rétribution"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Sovereign of the Day of Recompense"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Din Gününün sahibidir"
        }
      },
      "mots": [
        {
          "ar": "مَٰلِكِ",
          "translit": "māliki",
          "fr": "Maître / Possesseur",
          "racine": "م-ل-ك",
          "frequence_coran": 3
        },
        {
          "ar": "يَوْمِ",
          "translit": "yawmi",
          "fr": "du Jour",
          "racine": "ي-و-م",
          "frequence_coran": 405
        },
        {
          "ar": "الدِّينِ",
          "translit": "d-dīni",
          "fr": "de la rétribution",
          "racine": "د-ي-ن",
          "frequence_coran": 92
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 5,
      "ar": "إِيَّاكَ نَعْبُدُ وَإِيَّاكَ نَسْتَعِينُ",
      "translit": "Iyyāka naʿbudu waʾiyyāka nastaʿīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "C'est Toi [Seul] que nous adorons, et c'est Toi [Seul] dont nous implorons secours"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "It is You we worship and You we ask for help"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Ancak Sana kulluk eder ve yalnız Senden yardım dileriz"
        }
      },
      "mots": [
        {
          "ar": "إِيَّاكَ",
          "translit": "iyyāka",
          "fr": "Toi seul",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "نَعْبُدُ",
          "translit": "naʿbudu",
          "fr": "nous adorons",
          "racine": "ع-ب-د",
          "frequence_coran": 275,
          "famille": [
            "عَبْد (ʿabd) — serviteur",
            "عِبَادَة (ʿibāda) — adoration",
            "عِبَاد (ʿibād) — serviteurs"
          ],
          "profondeur": "Naʿbudu vient après iyyāka (Toi seul) : en arabe, placer le complément avant le verbe crée l'exclusivité. La phrase ne dit pas 'nous T'adorons' mais 'c'est Toi — et personne d'autre — que nous adorons'. Tout le tawhid pratique tient dans cet ordre des mots."
        },
        {
          "ar": "وَإِيَّاكَ",
          "translit": "waʾiyyāka",
          "fr": "et Toi seul",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "نَسْتَعِينُ",
          "translit": "nastaʿīnu",
          "fr": "nous implorons l'aide",
          "racine": "ع-و-ن",
          "frequence_coran": 11
        }
      ],
      "analyse_globale": "Verset-pivot de la sourate : les versets 1 à 4 parlent DE Dieu (louange), les versets 6 et 7 s'adressent À Dieu (demande). Le verset 5 fait la bascule — le serviteur passe de la contemplation au dialogue direct. Selon un hadith qudsi, Allah répond à chaque verset de la Fatiha récité par Son serviteur : ce verset est 'partagé entre Moi et Mon serviteur'.",
      "niveau_couche": 1
    },
    {
      "numero": 6,
      "ar": "اهْدِنَا الصِّرَٰطَ الْمُسْتَقِيمَ",
      "translit": "Ihdinā ṣ-ṣirāṭa l-mustaqīm",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "Guide-nous dans le droit chemin"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "Guide us to the straight path"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Bizi doğru yola eriştir"
        }
      },
      "mots": [
        {
          "ar": "اهْدِنَا",
          "translit": "ihdinā",
          "fr": "guide-nous",
          "racine": "ه-د-ي",
          "frequence_coran": 316,
          "famille": [
            "هُدًى (hudan) — guidance",
            "هَادٍ (hādin) — guide",
            "اهْتَدَىٰ (ihtadā) — se laisser guider"
          ],
          "profondeur": "Ihdinā est la demande centrale de la Fatiha — et donc de la prière. Après avoir loué (v.1-4) et déclaré l'adoration (v.5), le croyant ne demande qu'une seule chose : la guidance. Ni richesse, ni santé — la direction. La racine h-d-y apparaît ~316 fois : c'est LE fil conducteur du Coran."
        },
        {
          "ar": "الصِّرَٰطَ",
          "translit": "ṣ-ṣirāṭa",
          "fr": "le chemin",
          "racine": "ص-ر-ط",
          "frequence_coran": 45
        },
        {
          "ar": "الْمُسْتَقِيمَ",
          "translit": "l-mustaqīma",
          "fr": "droit",
          "racine": "ق-و-م",
          "frequence_coran": 37
        }
      ],
      "niveau_couche": 1
    },
    {
      "numero": 7,
      "ar": "صِرَٰطَ الَّذِينَ أَنْعَمْتَ عَلَيْهِمْ غَيْرِ الْمَغْضُوبِ عَلَيْهِمْ وَلَا الضَّالِّينَ",
      "translit": "Ṣirāṭa lladhīna anʿamta ʿalayhim ghayri l-maghḍūbi ʿalayhim walā ḍ-ḍāllīn",
      "traductions": {
        "fr": {
          "auteur": "Muhammad Hamidullah",
          "texte": "le chemin de ceux que Tu as comblés de faveurs, non pas de ceux qui ont encouru Ta colère, ni des égarés"
        },
        "en": {
          "auteur": "Saheeh International",
          "texte": "The path of those upon whom You have bestowed favor, not of those who have evoked [Your] anger or of those who are astray"
        },
        "tr": {
          "auteur": "Diyanet İşleri Başkanlığı",
          "texte": "Nimete erdirdiğin kimselerin yoluna; gazaba uğrayanların, ya da sapıtanların yoluna değil"
        }
      },
      "mots": [
        {
          "ar": "صِرَٰطَ",
          "translit": "ṣirāṭa",
          "fr": "chemin (de)",
          "racine": "ص-ر-ط",
          "frequence_coran": 45
        },
        {
          "ar": "الَّذِينَ",
          "translit": "lladhīna",
          "fr": "ceux que",
          "racine": "— (relatif)",
          "frequence_coran": 1080
        },
        {
          "ar": "أَنْعَمْتَ",
          "translit": "anʿamta",
          "fr": "Tu as comblés",
          "racine": "ن-ع-م",
          "frequence_coran": 140
        },
        {
          "ar": "عَلَيْهِمْ",
          "translit": "ʿalayhim",
          "fr": "envers eux",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "غَيْرِ",
          "translit": "ghayri",
          "fr": "autre que / non",
          "racine": "غ-ي-ر",
          "frequence_coran": 147
        },
        {
          "ar": "الْمَغْضُوبِ",
          "translit": "l-maghḍūbi",
          "fr": "ceux qui ont encouru la colère",
          "racine": "غ-ض-ب",
          "frequence_coran": 24
        },
        {
          "ar": "عَلَيْهِمْ",
          "translit": "ʿalayhim",
          "fr": "sur eux",
          "racine": "— (pronom)",
          "frequence_coran": 0
        },
        {
          "ar": "وَلَا",
          "translit": "walā",
          "fr": "et non",
          "racine": "— (négation)",
          "frequence_coran": 0
        },
        {
          "ar": "الضَّالِّينَ",
          "translit": "ḍ-ḍāllīna",
          "fr": "les égarés",
          "racine": "ض-ل-ل",
          "frequence_coran": 191
        }
      ],
      "analyse_globale": "Trois chemins sont décrits : celui des comblés (les prophètes, les véridiques, les martyrs, les vertueux — cf. 4:69), celui de la colère (savoir sans agir) et celui de l'égarement (agir sans savoir). La guidance demandée au v.6 se précise : marcher avec ceux qui savent ET agissent.",
      "niveau_couche": 1
    }
  ],
  "resume_pedagogique": {
    "mots_cles_a_retenir": [
      "الْحَمْدُ",
      "الرَّحْمَٰنِ",
      "نَعْبُدُ",
      "اهْدِنَا",
      "الصِّرَٰطَ"
    ],
    "racines_a_retenir": [
      "ح-م-د",
      "ر-ح-م",
      "ع-ب-د",
      "ه-د-ي"
    ],
    "declic_frequence": "Avec seulement 25 mots, la Fatiha contient rabb (~970 occurrences), Allah (2699), raḥmān/raḥīm (~172), dīn (92), ʿabada (~275) et hadā (~316) : réciter la Fatiha en la comprenant, c'est déjà activer plus de 4500 occurrences du texte coranique."
  }
};

export { S001 };
